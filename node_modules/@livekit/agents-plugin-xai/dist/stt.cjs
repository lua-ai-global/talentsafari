"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var stt_exports = {};
__export(stt_exports, {
  STT: () => STT,
  SpeechStream: () => SpeechStream
});
module.exports = __toCommonJS(stt_exports);
var import_agents = require("@livekit/agents");
var import_node_crypto = require("node:crypto");
var import_ws = require("ws");
var import_utils = require("./_utils.cjs");
const SAMPLE_RATE = 16e3;
const XAI_WEBSOCKET_URL = "wss://api.x.ai/v1/stt";
const XAI_REST_URL = "https://api.x.ai/v1/stt";
const defaultSTTOptions = {
  interimResults: true,
  sampleRate: SAMPLE_RATE,
  enableDiarization: false,
  language: "en",
  endpointing: 100
};
class STT extends import_agents.stt.STT {
  #opts;
  #apiKey;
  #streams = /* @__PURE__ */ new Set();
  label = "xai.STT";
  get provider() {
    return "xai";
  }
  constructor(opts = {}) {
    const merged = { ...defaultSTTOptions, ...opts };
    super({
      streaming: true,
      interimResults: merged.interimResults,
      alignedTranscript: "word"
    });
    const apiKey = opts.apiKey ?? process.env.XAI_API_KEY;
    if (!apiKey) {
      throw new Error("xAI API key is required, whether as an argument or as $XAI_API_KEY");
    }
    this.#apiKey = apiKey;
    this.#opts = { ...merged, apiKey };
  }
  async _recognize(buffer) {
    const frame = (0, import_agents.mergeFrames)(buffer);
    const wavBuffer = createWav(frame);
    const file = new File([new Uint8Array(wavBuffer)], "audio.wav", { type: "audio/wav" });
    const formData = new FormData();
    formData.append("file", file);
    formData.append("language", this.#opts.language);
    formData.append("format", "true");
    const resp = await fetch(XAI_REST_URL, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${this.#apiKey}`,
        Accept: "application/json"
      },
      body: formData
    });
    if (!resp.ok) {
      throw new Error(`xAI STT REST request failed with status ${resp.status}`);
    }
    const data = await resp.json();
    return prerecordedTranscriptionToSpeechEvent(data);
  }
  stream(options) {
    const stream = new SpeechStream(this, this.#opts, this.#apiKey, options == null ? void 0 : options.connOptions);
    this.#streams.add(new WeakRef(stream));
    return stream;
  }
  updateOptions(opts) {
    this.#opts = { ...this.#opts, ...opts };
    for (const ref of this.#streams) {
      const stream = ref.deref();
      if (stream) {
        stream.updateOptions(opts);
      } else {
        this.#streams.delete(ref);
      }
    }
  }
  async close() {
  }
}
class SpeechStream extends import_agents.stt.SpeechStream {
  #opts;
  #apiKey;
  #logger = (0, import_agents.log)();
  #speaking = false;
  #emittedChunkFinal = false;
  #resetWS = new import_agents.Future();
  #requestId;
  #audioDurationCollector;
  #serverReady = new import_agents.Future();
  label = "xai.SpeechStream";
  constructor(stt2, opts, apiKey, connOptions) {
    super(stt2, opts.sampleRate, connOptions);
    this.#opts = { ...opts };
    this.#apiKey = apiKey;
    this.closed = false;
    this.#requestId = (0, import_node_crypto.randomUUID)();
    this.#audioDurationCollector = new import_utils.PeriodicCollector(
      (duration) => this.#onAudioDurationReport(duration),
      { duration: 5 }
    );
  }
  updateOptions(opts) {
    this.#opts = { ...this.#opts, ...opts };
    this.#resetWS.resolve();
  }
  async run() {
    const maxRetry = 32;
    let retries = 0;
    let ws;
    while (!this.input.closed && !this.closed) {
      const streamURL = new URL(XAI_WEBSOCKET_URL);
      streamURL.searchParams.set("encoding", "pcm");
      streamURL.searchParams.set("sample_rate", String(this.#opts.sampleRate));
      streamURL.searchParams.set(
        "interim_results",
        String(this.#opts.interimResults).toLowerCase()
      );
      streamURL.searchParams.set("diarize", String(this.#opts.enableDiarization).toLowerCase());
      streamURL.searchParams.set("language", this.#opts.language);
      streamURL.searchParams.set("endpointing", String(this.#opts.endpointing));
      ws = new import_ws.WebSocket(streamURL, {
        headers: { Authorization: `Bearer ${this.#apiKey}` }
      });
      try {
        await new Promise((resolve, reject) => {
          ws.on("open", resolve);
          ws.on("error", (error) => reject(error));
          ws.on("close", (code) => reject(`WebSocket returned ${code}`));
        });
        await this.#runWS(ws);
      } catch (e) {
        if (!this.closed && !this.input.closed) {
          if (retries >= maxRetry) {
            throw new Error(`failed to connect to xAI after ${retries} attempts: ${e}`);
          }
          const delay = Math.min(retries * 5, 10);
          retries++;
          this.#logger.warn(
            `failed to connect to xAI STT, retrying in ${delay} seconds: ${e} (${retries}/${maxRetry})`
          );
          await new Promise((resolve) => setTimeout(resolve, delay * 1e3));
        } else {
          this.#logger.warn(
            `xAI STT disconnected, connection is closed: ${e} (inputClosed: ${this.input.closed}, isClosed: ${this.closed})`
          );
        }
      }
    }
    this.closed = true;
  }
  async #runWS(ws) {
    this.#resetWS = new import_agents.Future();
    this.#serverReady = new import_agents.Future();
    let closing = false;
    const wsMonitor = import_agents.Task.from(async (controller) => {
      const closed = new Promise(async (_, reject) => {
        ws.once("close", (code, reason) => {
          if (!closing) {
            this.#logger.error(`WebSocket closed with code ${code}: ${reason}`);
            reject(new Error("WebSocket closed"));
          }
        });
      });
      await Promise.race([closed, (0, import_agents.waitForAbort)(controller.signal)]);
    });
    const sendTask = async () => {
      await this.#serverReady.await;
      const samples50ms = Math.floor(this.#opts.sampleRate / 20);
      const stream = new import_agents.AudioByteStream(this.#opts.sampleRate, 1, samples50ms);
      const abortPromise = (0, import_agents.waitForAbort)(this.abortSignal);
      try {
        while (!this.closed) {
          const result = await Promise.race([this.input.next(), abortPromise]);
          if (result === void 0) return;
          if (result.done) break;
          const data = result.value;
          let frames;
          if (data === SpeechStream.FLUSH_SENTINEL) {
            frames = stream.flush();
            this.#audioDurationCollector.flush();
          } else {
            frames = stream.write(data.data.buffer);
          }
          for (const frame of frames) {
            const frameDuration = frame.samplesPerChannel / frame.sampleRate;
            this.#audioDurationCollector.push(frameDuration);
            ws.send(frame.data.buffer);
          }
        }
      } finally {
        this.#audioDurationCollector.flush();
        closing = true;
        ws.send(JSON.stringify({ type: "audio.done" }));
        wsMonitor.cancel();
      }
    };
    const listenTask = import_agents.Task.from(async (controller) => {
      const listenMessage = new Promise((resolve, reject) => {
        ws.on("message", (msg) => {
          try {
            const json = JSON.parse(msg.toString());
            this.#processStreamEvent(json);
            if (this.closed || closing) {
              resolve();
            }
          } catch (err) {
            this.#logger.error(`xAI STT: error processing message: ${msg}`);
            reject(err);
          }
        });
      });
      await Promise.race([listenMessage, (0, import_agents.waitForAbort)(controller.signal)]);
    }, this.abortController);
    await Promise.race([
      this.#resetWS.await,
      Promise.all([sendTask(), listenTask.result, wsMonitor])
    ]);
    closing = true;
    ws.close();
  }
  #onAudioDurationReport(duration) {
    const usageEvent = {
      type: import_agents.stt.SpeechEventType.RECOGNITION_USAGE,
      requestId: this.#requestId,
      recognitionUsage: {
        audioDuration: duration
      }
    };
    if (!this.queue.closed) {
      this.queue.put(usageEvent);
    }
  }
  #putMessage(message) {
    if (!this.queue.closed) {
      try {
        this.queue.put(message);
      } catch {
      }
    }
  }
  #processStreamEvent(data) {
    const msgType = data["type"] ?? "";
    if (msgType === "transcript.created") {
      this.#serverReady.resolve();
    } else if (msgType === "transcript.partial") {
      const text = data["text"] ?? "";
      const isFinal = data["is_final"] ?? false;
      const speechFinal = data["speech_final"] ?? false;
      const words = data["words"] ?? [];
      const language = data["language"] ?? "";
      if (!text) return;
      if (!this.#speaking) {
        this.#speaking = true;
        this.#putMessage({ type: import_agents.stt.SpeechEventType.START_OF_SPEECH });
      }
      if (isFinal) {
        if (!speechFinal) {
          this.#emittedChunkFinal = true;
          this.#putMessage({
            type: import_agents.stt.SpeechEventType.FINAL_TRANSCRIPT,
            requestId: this.#requestId,
            alternatives: [wordsToSpeechData(words, text, language)]
          });
        } else {
          if (!this.#emittedChunkFinal) {
            this.#putMessage({
              type: import_agents.stt.SpeechEventType.FINAL_TRANSCRIPT,
              requestId: this.#requestId,
              alternatives: [wordsToSpeechData(words, text, language)]
            });
          }
          this.#emittedChunkFinal = false;
          this.#speaking = false;
          this.#putMessage({ type: import_agents.stt.SpeechEventType.END_OF_SPEECH });
        }
      } else {
        if (this.#opts.interimResults) {
          this.#putMessage({
            type: import_agents.stt.SpeechEventType.INTERIM_TRANSCRIPT,
            requestId: this.#requestId,
            alternatives: [
              {
                language,
                text,
                startTime: 0,
                endTime: 0,
                confidence: 0
              }
            ]
          });
        }
      }
    } else if (msgType === "transcript.done") {
      const text = data["text"] ?? "";
      const words = data["words"] ?? [];
      const language = data["language"] ?? "";
      if (text) {
        this.#putMessage({
          type: import_agents.stt.SpeechEventType.FINAL_TRANSCRIPT,
          requestId: this.#requestId,
          alternatives: [wordsToSpeechData(words, text, language)]
        });
      }
      if (this.#speaking) {
        this.#speaking = false;
        this.#putMessage({ type: import_agents.stt.SpeechEventType.END_OF_SPEECH });
      }
    } else if (msgType === "error") {
      this.#logger.error(`xAI STT error: ${data["message"] ?? "unknown error"}`);
    } else {
      this.#logger.warn(`received unexpected message from xAI: ${msgType}`);
    }
  }
}
function wordsToSpeechData(words, text, language) {
  return {
    language,
    text,
    startTime: words.length ? words[0]["start"] ?? 0 : 0,
    endTime: words.length ? words[words.length - 1]["end"] ?? 0 : 0,
    confidence: 0,
    words: words.length > 0 ? words.map(
      (w) => (0, import_agents.createTimedString)({
        text: w["text"] ?? "",
        startTime: w["start"] ?? 0,
        endTime: w["end"] ?? 0
      })
    ) : void 0
  };
}
function prerecordedTranscriptionToSpeechEvent(data) {
  const text = data["text"] ?? "";
  const words = data["words"] ?? [];
  const language = data["language"] ?? "";
  return {
    type: import_agents.stt.SpeechEventType.FINAL_TRANSCRIPT,
    alternatives: [wordsToSpeechData(words, text, language)]
  };
}
function createWav(frame) {
  const bitsPerSample = 16;
  const byteRate = frame.sampleRate * frame.channels * bitsPerSample / 8;
  const blockAlign = frame.channels * bitsPerSample / 8;
  const header = Buffer.alloc(44);
  header.write("RIFF", 0);
  header.writeUInt32LE(36 + frame.data.byteLength, 4);
  header.write("WAVE", 8);
  header.write("fmt ", 12);
  header.writeUInt32LE(16, 16);
  header.writeUInt16LE(1, 20);
  header.writeUInt16LE(frame.channels, 22);
  header.writeUInt32LE(frame.sampleRate, 24);
  header.writeUInt32LE(byteRate, 28);
  header.writeUInt16LE(blockAlign, 32);
  header.writeUInt16LE(16, 34);
  header.write("data", 36);
  header.writeUInt32LE(frame.data.byteLength, 40);
  const pcm = Buffer.from(frame.data.buffer, frame.data.byteOffset, frame.data.byteLength);
  return Buffer.concat([header, pcm]);
}
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  STT,
  SpeechStream
});
//# sourceMappingURL=stt.cjs.map