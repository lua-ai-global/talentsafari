...
class STTv2 extends import_agents.stt.STT {
  ...
  constructor(opts = {}) {
    ...
    this.#opts = {
      ...defaultSTTv2Options,
      ...opts,
      language: opts.language ? (0, import_agents.normalizeLanguage)(opts.language) : defaultSTTv2Options.language
    };
    ...
    if (this.#opts.languageHint && this.#opts.languageHint.length > 0 && this.#opts.model !== "flux-general-multi") {
      this.#logger.warn(
  ...
  updateOptions(opts) {
    this.#opts = {
      ...this.#opts,
      ...opts,
      language: opts.language !== void 0 ? (0, import_agents.normalizeLanguage)(opts.language) : this.#opts.language
    };
...
class SpeechStreamv2 extends import_agents.stt.SpeechStream {
  ...
  async #sendTask() {
    ...
    while (true) {
      ...
      if (hasEnded && result.value === void 0) {
...
function parseTranscription(language, data, startTimeOffset) {
  ...
  const detectedLanguagesRaw = Array.isArray(data.languages) ? data.languages : [];
  const detectedLanguages = detectedLanguagesRaw.map((lang) => (0, import_agents.normalizeLanguage)(lang));
  const primaryLanguage = detectedLanguages.length > 0 ? detectedLanguages[0] : (0, import_agents.normalizeLanguage)(language);
  const sd = {
    ...
    sourceLanguages: detectedLanguages.length > 0 ? detectedLanguages : void 0,
