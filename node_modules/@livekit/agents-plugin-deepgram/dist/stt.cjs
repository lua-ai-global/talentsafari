...
class STT extends import_agents.stt.STT {
  ...
  constructor(opts = defaultSTTOptions) {
    ...
    this.#opts = {
      ...defaultSTTOptions,
      ...opts,
      language: opts.language ? (0, import_agents.normalizeLanguage)(opts.language) : defaultSTTOptions.language
    };
    if (this.#opts.detectLanguage) {
      this.#opts.language = void 0;
    } else if (this.#opts.language && (0, import_agents.getBaseLanguage)(this.#opts.language) !== "en" && [
      "nova-2-meeting",
      "nova-2-phonecall",
      "nova-2-finance",
...
const liveTranscriptionToSpeechData = (language, data, startTimeOffset = 0) => {
  ...
  return alts.map((alt) => {
    ...
    const detectedLanguage = language === "multi" ? ((_a = alt["languages"]) == null ? void 0 : _a[0]) ?? language : alt["language"] ?? language;
    return {
      language: (0, import_agents.normalizeLanguage)(detectedLanguage),
