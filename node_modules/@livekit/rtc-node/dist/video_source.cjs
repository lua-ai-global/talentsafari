...
class VideoSource {
  constructor(width, height) {
    ...
    this.width = width;
    this.height = height;
    const req = new import_rtc_ffi_bindings.NewVideoSourceRequest({
      type: import_rtc_ffi_bindings.VideoSourceType.VIDEO_SOURCE_NATIVE,
      resolution: {
        width,
        height
      }
  ...
  captureFrame(frame, timestampUs = BigInt(0), rotation = import_rtc_ffi_bindings.VideoRotation.VIDEO_ROTATION_0) {
    if (this.closed) {
      throw new Error("VideoSource is closed");
    }
    const req = new import_rtc_ffi_bindings.CaptureVideoFrameRequest({
