...
const defaultRtcConfiguration = {
  iceTransportType: import_rtc_ffi_bindings3.IceTransportType.TRANSPORT_ALL,
  continualGatheringPolicy: import_rtc_ffi_bindings3.ContinualGatheringPolicy.GATHER_CONTINUALLY,
  iceServers: []
};
const defaultRoomOptions = new import_rtc_ffi_bindings3.RoomOptions({
  autoSubscribe: true,
  dynacast: false,
  e2ee: void 0,
  encryption: void 0,
...
class Room extends import_events.default {
  constructor() {
    ...
    this._connectionState = import_rtc_ffi_bindings3.ConnectionState.CONN_DISCONNECTED;
    ...
    this.processFfiEvent = async (ffiEvent) => {
      ...
      if (ev.case == "participantConnected") {
        ...
        if (publication) {
          ...
          import_log.log.warn(`RoomEvent.LocalTrackRepublished: previous publication not found: ${previousSid}`);
        }
        ...
        try {
          ...
          if (trackInfo.kind == import_rtc_ffi_bindings4.TrackKind.KIND_VIDEO) {
            publication.track = new import_track.RemoteVideoTrack(ownedTrack);
          } else if (trackInfo.kind == import_rtc_ffi_bindings4.TrackKind.KIND_AUDIO) {
        ...
        if (ev.value.state == import_rtc_ffi_bindings.EncryptionState.INTERNAL_ERROR) {
  ...
  get isConnected() {
    return this.ffiHandle != void 0 && this.connectionState != import_rtc_ffi_bindings3.ConnectionState.CONN_DISCONNECTED;
  }
  /** @internal */
  get token() {
    return this._token;
  ...
  async connect(url, token, opts) {
    ...
    const req = new import_rtc_ffi_bindings3.ConnectRequest({
      url,
      token,
      options
    });
    ...
    try {
      ...
      import_log.log.debug("Connect callback received");
      switch (cb.message.case) {
        case "result":
          ...
          this.updateConnectionState(import_rtc_ffi_bindings3.ConnectionState.CONN_CONNECTED);
          break;
        case "error":
        default:
      ...
      import_ffi_client.FfiClient.instance.off(import_ffi_client.FfiClientEvent.FfiEvent, this.onFfiEvent);
      this.preConnectEvents = [];
      throw e;
    }
  }
  ...
  async disconnect() {
    ...
    this.cleanupOnDisconnect(import_rtc_ffi_bindings2.DisconnectReason.CLIENT_INITIATED);
  ...
  cleanupOnDisconnect(reason = import_rtc_ffi_bindings2.DisconnectReason.CLIENT_INITIATED) {
    ...
    this.updateConnectionState(import_rtc_ffi_bindings3.ConnectionState.CONN_DISCONNECTED);
