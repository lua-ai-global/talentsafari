...
class AudioSource {
  constructor(sampleRate, numChannels, queueSize = 1e3) {
    ...
    const req = new import_rtc_ffi_bindings.NewAudioSourceRequest({
      type: import_rtc_ffi_bindings.AudioSourceType.AUDIO_SOURCE_NATIVE,
      sampleRate,
      numChannels,
    ...
    this.ffiHandle = new import_rtc_ffi_bindings.FfiHandle(res.source.handle.id);
  }
  get queuedDuration() {
    return Math.max(
  ...
  clearQueue() {
    const req = new import_rtc_ffi_bindings.ClearAudioBufferRequest({
