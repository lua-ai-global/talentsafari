...
class STT extends stt.STT {
  ...
  constructor(opts = {}) {
    ...
    this.#opts = {
      ...
      languageCode: opts.languageCode ? normalizeLanguage(opts.languageCode) : void 0,
    ...
    for (let i = 0; i < connOptions.maxRetry + 1; i++) {
      try {
        ...
        this.emit("metrics_collected", {
          ...
          durationMs,
          label: this.label,
          audioDurationMs: Math.round(calculateAudioDurationSeconds(buffer) * 1e3),
          streamed: false,
          metadata: {
  ...
  stream(options) {
    const stream = new SpeechStream(
      this,
      this.#opts,
      (options == null ? void 0 : options.connOptions) ?? DEFAULT_API_CONNECT_OPTIONS,
      (options == null ? void 0 : options.language) !== void 0 ? normalizeLanguage(options.language) : this.#opts.languageCode,
      this.#session
    );
...
class SpeechStream extends stt.SpeechStream {
      ...
      try {
        ...
        const keepaliveTask = Task.from(async (controller) => {
          while (!controller.signal.aborted) {
            await Promise.race([delay(1e4), waitForAbort(controller.signal)]);
            if (controller.signal.aborted || (ws == null ? void 0 : ws.readyState) !== WebSocket.OPEN) return;
            ws.send(
              JSON.stringify({
        ...
        const runResult = await Promise.race([
          ...
          waitForAbort(this.abortSignal).then(() => "abort")
        ]);
        if (runResult === "reconnect") {
          this.#reconnectEvent = new Future();
          continue;
        }
        break;
      } finally {
  ...
  #processStreamEvent(data) {
    ...
    const normalizedLanguage = languageCode ? normalizeLanguage(languageCode) : normalizeLanguage("en");
    const speechData = {
