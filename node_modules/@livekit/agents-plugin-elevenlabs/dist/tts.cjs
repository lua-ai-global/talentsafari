...
function toTimedWords(text, startTimesMs, durationsMs, flush = false, firstWordOffsetMs = 0) {
  ...
  const words = import_agents.tokenize.basic.splitWords(text, false);
  const timedWords = [];
  if (words.length === 0) {
    return [[], text];
  }
...
class Connection {
  ...
  async connect() {
    ...
    return new Promise((resolve, reject) => {
      this.#ws = new import_ws.WebSocket(url, { headers });
      this.#ws.on("open", () => {
      ...
      this.#ws.on("error", (error) => {
        ...
        reject(new import_agents.APIConnectionError({ message: `WebSocket error: ${error.message}` }));
      });
    });
  }
  ...
  sendContent(content) {
    var _a;
    if (this.#closed || !this.#ws || this.#ws.readyState !== import_ws.WebSocket.OPEN) {
      throw new import_agents.APIConnectionError({ message: "WebSocket connection is closed" });
    }
    ...
    try {
      while (!this.#closed) {
        ...
        if (data.error) {
          ...
          if (contextId) {
            if (ctx) {
              ctx.waiter.reject(new import_agents.APIError(data.error));
            }
      ...
      for (const ctx of this.#contextData.values()) {
        ctx.waiter.reject((0, import_agents.asError)(e));
      }
  ...
  async close() {
    ...
    (_a = this.#inputQueueResolver) == null ? void 0 : _a.call(this);
    for (const ctx of this.#contextData.values()) {
      ctx.waiter.reject(new import_agents.APIStatusError({ message: "connection closed" }));
    }
    this.#contextData.clear();
    if (this.#ws) {
      this.#ws.close();
...
class TTS extends import_agents.tts.TTS {
  ...
  #connectionLock = new import_mutex.Mutex();
  #logger = (0, import_agents.log)();
...
class ChunkedStream extends import_agents.tts.ChunkedStream {
  ...
  async run() {
    ...
    const bstream = new import_agents.AudioByteStream(this.#opts.sampleRate, 1);
    try {
      ...
      if (!contentType.startsWith("audio/")) {
        ...
        throw new import_agents.APIError(`ElevenLabs returned non-audio data: ${content}`);
      }
      const reader = (_a = response.body) == null ? void 0 : _a.getReader();
      if (!reader) {
        throw new import_agents.APIError("No response body");
      }
      let lastFrame;
      while (true) {
      ...
      if (e instanceof import_agents.APIError) {
        throw e;
      }
      if (e instanceof Error && e.name === "AbortError") {
        return;
      }
      throw new import_agents.APIConnectionError({ message: `Connection error: ${e}` });
    }
  }
}
class SynthesizeStream extends import_agents.tts.SynthesizeStream {
  ...
  constructor(tts2, opts, connOptions) {
    ...
    this.#tts = tts2;
    this.#opts = opts;
    this.#contextId = (0, import_agents.shortuuid)();
  ...
  async run() {
    ...
    try {
      ...
      throw new import_agents.APIConnectionError({ message: "could not connect to ElevenLabs" });
    }
    let waiterReject;
    ...
    const sentenceStreamTask = async () => {
      const flushOnChunk = this.#opts.wordTokenizer instanceof import_agents.tokenize.SentenceTokenizer && this.#opts.autoMode;
      let xmlContent = [];
    ...
    try {
      ...
      if (e instanceof import_agents.APITimeoutError) {
        throw e;
      }
      if (e instanceof import_agents.APIStatusError) {
        throw e;
      }
      throw new import_agents.APIStatusError({ message: "Could not synthesize" });
    } finally {
