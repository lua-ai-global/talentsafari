...
export function checkSourceFile(sourceFile: ts.SourceFile, checker: ts.TypeChecker): CheckResult[] {
...
function checkThrowStatement(
  ...
  if (!isAllowed) {
    ...
    const declaredNames = declaredErrors.map((t) => checker.typeToString(t)).join(' | ');
...
function checkReturnStatement(
  ...
  // If the return is inside a try block with a catch clause, errors from the
  // returned expression will be caught at runtime — they don't propagate through
  // the return.  Error propagation from the catch is already checked separately
  // by checkThrowStatement / checkCallExpression.
  const tryCatch = getContainingTryCatch(node);
...
function getDeclaredErrorTypes(
  ...
  if (!throwsProperty) {
    ...
    if (promiseType) {
      ...
      if (!innerThrows) {
        // When T is a primitive (e.g. Promise<Throws<string, E>>), TypeScript distributes
        // the inner type into a union: `string | (string & { __throws?: E })`.
        if (promiseType.isUnion()) {
          const memberErrors = promiseType.types.flatMap((t) =>
            extractThrowsErrorTypes(t, checker),
          );
    ...
    // When T is a primitive (e.g. Throws<string, E>), TypeScript distributes the
    // intersection into a union: `string | (string & { __throws?: E })`.  The
    // top-level union won't have __throws, but one of its members will.
    if (returnType.isUnion()) {
      const memberErrors = returnType.types.flatMap((t) => extractThrowsErrorTypes(t, checker));
      if (memberErrors.length > 0) {
        return memberErrors;
      }
    }
...
function isPromiseCallNotImmediatelyConsumed(
  node: ts.CallExpression,
  checker: ts.TypeChecker,
): boolean {
...
function extractThrowsErrorTypes(type: ts.Type, checker: ts.TypeChecker): ts.Type[] {
  ...
  const throwsType = checker.getTypeOfSymbolAtLocation(throwsProperty, declaration);

  // __throws is a method signature `__throws?(error: E): void` for bivariant
  // checking.  The resolved type is `((error: E) => void) | undefined` because
  // the method is optional.  Find the function member and extract E from its
  // first parameter.
  const fnType = throwsType.isUnion()
    ? throwsType.types.find((t) => t.getCallSignatures().length > 0)
    : throwsType.getCallSignatures().length > 0
      ? throwsType
      : undefined;

  if (fnType) {
    const params = fnType.getCallSignatures()[0].getParameters();
    if (params.length > 0) {
      const paramType = checker.getTypeOfSymbolAtLocation(params[0], declaration);
      ...
      } else if (!isUndefinedType(paramType) && !isNeverType(paramType)) {
        errors.push(paramType);
      }
    }
    return errors;
  }

  // Fallback: property brand (readonly __throws?: E)
...
function extractPromiseType(type: ts.Type, checker: ts.TypeChecker): ts.Type | null {
  ...
  // Check if the type extends Promise (e.g. TypedPromise<T, E> extends Promise<Throws<T, E>>)
  // Uses TypeScript's own promise resolution to get the resolved inner type.
  const promisedType = (checker as any).getPromisedTypeOfPromise?.(type) as ts.Type | undefined;
  if (promisedType) {
    return promisedType;
  }
...
function getContainingFunction(node: ts.Node): ts.FunctionLikeDeclaration | null {
...
function getTryCatchThrownErrors(
  tryCatch: ts.TryStatement,
  sourceFile: ts.SourceFile,
  checker: ts.TypeChecker,
) {
...
function findNarrowedErrorTypes(
  ...
  function visitIfStatement(ifStmt: ts.IfStatement) {
    ...
    if (thenRethrows) {
      ...
      const narrowedType = getNarrowedTypeInBlock(thenStatement, errorVarName, checker);
    ...
    if (ifStmt.elseStatement) {
      if (ts.isIfStatement(ifStmt.elseStatement)) {
        ...
        if (elseHandles) {
          const narrowedType = getNarrowedTypeInBlock(ifStmt.elseStatement, errorVarName, checker);
          if (narrowedType && !isAnyOrUnknownType(narrowedType)) {
            if (narrowedType.isUnion()) {
              narrowedTypes.push(...narrowedType.types.filter((t) => !isAnyOrUnknownType(t)));
            } else {
 ...
 */
function findInstanceofChecks(block: ts.Block, errorVar: ts.VariableDeclaration): ts.Expression[] {
