...
class StreamAdapterWrapper extends import_tts.SynthesizeStream {
  ...
  #sentenceStream;
  label;
  constructor(tts, sentenceTokenizer, connOptions) {
  ...
  async run() {
    ...
    const synthesizeSentenceStream = async () => {
      ...
      await import_throws.ThrowsPromise.all(tokenCompletionTasks.map((t) => t.result));
    ...
    await import_throws.ThrowsPromise.all([forwardInput(), synthesizeSentenceStream()]);
  }
}
