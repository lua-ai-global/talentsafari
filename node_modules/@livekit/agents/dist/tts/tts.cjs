...
class SynthesizeStream {
  ...
  constructor(tts, connOptions = import_types.DEFAULT_API_CONNECT_OPTIONS) {
    ...
    (0, import_utils.startSoon)(async () => {
      try {
        await this.mainTask();
      } catch {
      } finally {
        ...
        return await import_telemetry.tracer.startActiveSpan(
          async (attemptSpan) => {
            ...
            try {
              ...
              this.#currentAttemptSpan = void 0;
            }
          },
   ...
   */
  noteProviderRequestId(contextId) {
    ...
    if (this.#currentAttemptSpan && this.#currentAttemptSpan.isRecording()) {
      this.#currentAttemptSpan.setAttribute(
        import_telemetry.traceTypes.ATTR_PROVIDER_REQUEST_IDS,
        this.#providerRequestIds
      );
    }
  }
    ...
    const emit = () => {
      if (this.#metricsPendingTexts.length) {
        ...
        if (this.#ttsRequestSpan) {
          this.#ttsRequestSpan.setAttribute(import_telemetry.traceTypes.ATTR_TTS_METRICS, JSON.stringify(metrics));
        }
...
class ChunkedStream {
  ...
  constructor(text, tts, connOptions = import_types.DEFAULT_API_CONNECT_OPTIONS, abortSignal) {
    ...
    this.monitorMetrics();
    import_throws.ThrowsPromise.resolve().then(() => this.mainTask().finally(() => this.queue.close()));
  }
  _mainTaskImpl = async (span) => {
    this.#ttsRequestSpan = span;
    span.setAttributes({
      [import_telemetry.traceTypes.ATTR_TTS_STREAMING]: false,
      [import_telemetry.traceTypes.ATTR_TTS_LABEL]: this.#tts.label
    });
  ...
  async mainTask() {
    return import_telemetry.tracer.startActiveSpan(async (span) => this._mainTaskImpl(span), {
      name: "tts_request",
      endOnExit: false
    });
  }
