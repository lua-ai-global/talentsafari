...
class FallbackAdapter extends import_tts.TTS {
  ...
  async close() {
    ...
    await import_throws.ThrowsPromise.all(this.ttsInstances.map((tts) => tts.close()));
  }
}
      ...
      try {
        ...
        const forwardBufferToTTS = async () => {
          while (true) {
            ...
            await new import_throws.ThrowsPromise((resolve) => setTimeout(resolve, FORWARD_POLL_MS));
        ...
        const [outputResult, forwardBufferResult] = await import_throws.ThrowsPromise.allSettled([
          processOutput(),
