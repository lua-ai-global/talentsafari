...
var import_promise = require("@livekit/throws-transformer/promise");
...
class TextAudioSynchronizer extends import_node_events.EventEmitter {
  ...
  async #sleepIfNotClosed(delay) {
    await import_promise.ThrowsPromise.race([
      this.#closeFut.await,
      new import_promise.ThrowsPromise((resolve) => setTimeout(resolve, delay))
    ]);
  }
  #calcHyphens(text) {
    const hyphens = [];
