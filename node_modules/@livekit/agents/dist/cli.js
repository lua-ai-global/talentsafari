...
import { formatDownloadFailureMessage } from "./download.js";
...
const runApp = (opts) => {
  const logLevelOption = (defaultLevel) => new Option("--log-level <level>", "Set the logging level").choices(["trace", "debug", "info", "warn", "error", "fatal"]).default(defaultLevel).env("LOG_LEVEL");
  ...
  program.command("download-files").description("Download plugin dependency files").addOption(logLevelOption("debug")).action((...[, command]) => {
    ...
    const logger = log();
    logger.warn(
      "`download-files` via cli.runApp() is deprecated. Use `npx livekit-agents download-files` instead \u2014 it discovers installed plugins without loading your agent code, enabling better Docker layer caching."
    );
