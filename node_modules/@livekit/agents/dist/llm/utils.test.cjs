...
(0, import_vitest.describe)("validateChatContextStructure", () => {
  (0, import_vitest.it)("returns valid=true for well-formed chat context", () => {
    const ctx = new import_chat_context.ChatContext([
      import_chat_context.ChatMessage.create({
        id: "msg_user",
        role: "user",
    ...
    const result = (0, import_utils.validateChatContextStructure)(ctx);
    (0, import_vitest.expect)(result.valid).toBe(true);
    (0, import_vitest.expect)(result.errors).toBe(0);
    (0, import_vitest.expect)(result.warnings).toBe(0);
    (0, import_vitest.expect)(result.issues).toEqual([]);
  });
  (0, import_vitest.it)("detects duplicate ids and timestamp ordering issues", () => {
    const m1 = import_chat_context.ChatMessage.create({
      id: "dup_id",
      role: "user",
      content: ["hello"],
      createdAt: 10
    });
    const m2 = import_chat_context.ChatMessage.create({
      id: "dup_id",
      role: "assistant",
      content: ["world"],
      createdAt: 5
    });
    const ctx = new import_chat_context.ChatContext([m1, m2]);
    const result = (0, import_utils.validateChatContextStructure)(ctx);
    (0, import_vitest.expect)(result.valid).toBe(false);
    (0, import_vitest.expect)(result.errors).toBeGreaterThanOrEqual(2);
    (0, import_vitest.expect)(result.issues.some((i) => i.code === "duplicate_id")).toBe(true);
    (0, import_vitest.expect)(result.issues.some((i) => i.code === "timestamp_order")).toBe(true);
  });
  (0, import_vitest.it)("detects malformed terms and orphan function outputs", () => {
    const msg = import_chat_context.ChatMessage.create({
      id: "msg_1",
      role: "user",
      content: ["   "],
      createdAt: 1
    });
    const output = import_chat_context.FunctionCallOutput.create({
      id: "fn_out_1",
      callId: "call_missing",
      name: "lookup_order",
      output: "ok",
    ...
    const ctx = new import_chat_context.ChatContext([msg, output]);
    const result = (0, import_utils.validateChatContextStructure)(ctx);
    (0, import_vitest.expect)(result.valid).toBe(true);
    (0, import_vitest.expect)(result.errors).toBe(0);
    (0, import_vitest.expect)(result.warnings).toBeGreaterThanOrEqual(2);
    (0, import_vitest.expect)(result.issues.some((i) => i.code === "empty_text_term")).toBe(true);
    (0, import_vitest.expect)(result.issues.some((i) => i.code === "orphan_function_call_output")).toBe(true);
  });
});
