...
__export(chat_context_exports, {
  ...
  concatInstructions: () => concatInstructions,
...
function isInstructions(value) {
  return typeof value === "object" && value !== null && INSTRUCTIONS_SYMBOL in value && value[INSTRUCTIONS_SYMBOL] === true;
}
class Instructions {
  type = "instructions";
  _audioVariant;
  ...
  static tpl(strings, ...values) {
    ...
    const hasTextVariant = values.some(
      (value) => isInstructions(value) && value._textVariant !== void 0
    );
    return new Instructions({
      audio: render("audio"),
      text: hasTextVariant ? render("text") : void 0,
  ...
  concat(other) {
    if (isInstructions(other)) {
      const hasText = this._textVariant !== void 0 || other._textVariant !== void 0;
      return new Instructions({
        audio: this.audio + other.audio,
        text: hasText ? this.text + other.text : void 0,
        represent: this.value + other.value
      });
    }
    return new Instructions({
      audio: this.audio + other,
      text: this._textVariant !== void 0 ? this._textVariant + other : void 0,
  ...
  toJSON() {
    ...
    if (this._textVariant !== void 0) {
      result.text = this._textVariant;
    }
    return result;
  }
}
function renderInstructions(instructions, modality) {
  if (typeof instructions === "string") return instructions;
  return modality === void 0 ? instructions.value : instructions.asModality(modality).value;
}
...
function concatInstructions(...parts) {
...
class ChatMessage {
   ...
   */
  get textContent() {
    const parts = this.content.filter((c) => typeof c === "string" || isInstructions(c)).map((c) => typeof c === "string" ? c : c.value);
...
class AgentConfigUpdate {
  ...
  toJSON(excludeTimestamp = false) {
    ...
    if (this.instructions !== void 0) {
      result.instructions = isInstructions(this.instructions) ? this.instructions.toJSON() : this.instructions;
    }
    if (this.toolsAdded !== void 0) {
      result.toolsAdded = this.toolsAdded;
    }
    if (this.toolsRemoved !== void 0) {
  ...
  compareContent(a, b) {
    ...
    for (let i = 0; i < a.length; i++) {
      ...
      if (isInstructions(contentA) && isInstructions(contentB)) {
        if (contentA.audio !== contentB.audio || contentA.text !== contentB.text || contentA.value !== contentB.value) {
          return false;
        }
        continue;
      }
