...
(0, import_vitest.describe)("Google Provider Format - toChatCtx", () => {
  ...
  (0, import_vitest.it)("should render Instructions as their resolved value", async () => {
    const ctx = import_chat_context.ChatContext.empty();
    ctx.addMessage({
      role: "system",
      content: [
    ...
    ctx.addMessage({ role: "user", content: "Hello" });
    const [, formatData] = await (0, import_google.toChatCtx)(ctx, false);
    (0, import_vitest.expect)(formatData.systemMessages).toEqual(["text instructions"]);
  });
