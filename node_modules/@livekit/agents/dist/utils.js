...
const isPending = async (promise) => {
  ...
  const result = await Promise.race([
    ThrowsPromise.fromPromise(promise),
...
class Event {
  ...
  async wait() {
    ...
    let resolve = noop;
    const waiter = new ThrowsPromise((r) => {
      resolve = r;
...
class Task {
  ...
  static currentTaskStorage = new AsyncLocalStorage();
  resultFuture;
...
function withResolvers() {
  let resolve;
  let reject;
  const promise = new ThrowsPromise((res, rej) => {
    resolve = res;
    reject = rej;
  });
...
function delay(ms, options = {}) {
  ...
  if (signal == null ? void 0 : signal.aborted) return ThrowsPromise.reject(signal.reason ?? new Error("delay aborted"));
