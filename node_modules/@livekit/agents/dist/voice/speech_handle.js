...
class SpeechHandleCircularWaitError extends Error {
  constructor(functionCallName) {
    super(dedent`
...
const DEFAULT_INPUT_DETAILS = { modality: "audio" };
class SpeechHandle {
  constructor(_id, _allowInterruptions, _stepIndex, _inputDetails = DEFAULT_INPUT_DETAILS, parent) {
    this._id = _id;
    this._allowInterruptions = _allowInterruptions;
    this._stepIndex = _stepIndex;
    this._inputDetails = _inputDetails;
    this.parent = parent;
  ...
  async waitForPlayout() {
    ...
    if ((store == null ? void 0 : store.functionCall) && store.speechHandle === this) {
  ...
  then(onFulfilled, onRejected) {
    return this.waitForPlayout().then(() => {
      this.then = void 0;
      try {
        return onFulfilled ? onFulfilled(this) : this;
      } finally {
        delete this.then;
      }
    }, onRejected);
