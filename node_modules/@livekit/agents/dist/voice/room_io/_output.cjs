...
class ParticipantTranscriptionOutput extends BaseParticipantTranscriptionOutput {
  ...
  flushTask = null;
  jsonFormat;
  constructor(room, isDeltaStream, participant, options = {}) {
  ...
  async captureText(text) {
    ...
    const payload = this.jsonFormat ? this.encodeJsonChunk(text) : (0, import_io.isTimedString)(text) ? text.text : text;
    this.latestText = payload;
    await this.handleCaptureText(payload);
  }
  encodeJsonChunk(text) {
    const isTimed = (0, import_io.isTimedString)(text);
    const message = new import_protocol.AgentSession.TimedString({
      text: isTimed ? text.text : text,
      startTime: isTimed ? text.startTime : void 0,
      endTime: isTimed ? text.endTime : void 0,
      confidence: isTimed ? text.confidence : void 0,
      startTimeOffset: isTimed ? text.startTimeOffset : void 0
    });
...
class ParticipantAudioOutput extends import_io.AudioOutput {
  ...
  constructor(room, options) {
    ...
    this.audioSource = new import_rtc_node.AudioSource(
      options.sampleRate,
      options.numChannels,
      options.queueSizeMs
    );
