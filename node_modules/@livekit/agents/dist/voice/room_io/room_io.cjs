...
class RoomIO {
  ...
  constructor({
    ...
    this.jobContext = (0, import_job.getJobContext)(false);
  ...
  onConversationItemAdded = (ev) => {
    var _a;
    if (ev.item.type !== "agent_handoff" || !this.transcriptionSynchronizer) {
      return;
    }
    const sessionLlm = ((_a = this.agentSession.currentAgent) == null ? void 0 : _a.llm) ?? this.agentSession.llm;
    const nativeTranscriptSync = sessionLlm instanceof import_llm.RealtimeModel && !!sessionLlm.capabilities.nativeTranscriptSync;
  ...
  onAgentSessionClose = () => {
    ...
    if (!jobContext) {
      jobContext = (0, import_job.getJobContext)(false);
    }
    if (!jobContext) {
      return;
    }
    this.logger.info(
      { room: this.room.name },
      "deleting room on agent session close (disable via `RoomInputOptions.deleteRoomOnClose=false`)"
    );
    const deleteRoomTask = import_utils.Task.from(async () => {
  ...
  get localParticipant() {
    return this.room.localParticipant ?? void 0;
  }
  ...
  start() {
    ...
    if (this.outputOptions.transcriptionEnabled) {
      ...
      if (this.outputOptions.syncTranscription && audioOutput) {
        ...
        this.transcriptionSynchronizer = new import_synchronizer.TranscriptionSynchronizer(
          ...
          { ...import_synchronizer.defaultTextSyncOptions, enabled: !nativeTranscriptSync }
        );
      }
    }
    ...
    this.agentSession.on(import_events.AgentSessionEventTypes.Close, this.onAgentSessionClose);
    this.agentSession.on(
      import_events.AgentSessionEventTypes.ConversationItemAdded,
  ...
  async close() {
    ...
    this.agentSession.off(import_events.AgentSessionEventTypes.Close, this.onAgentSessionClose);
    ...
    if (this.deleteRoomTask) {
      try {
        await (0, import_utils.waitUntilTimeout)(this.deleteRoomTask.result, import_types.DEFAULT_API_CONNECT_OPTIONS.timeoutMs);
      } catch (error) {
