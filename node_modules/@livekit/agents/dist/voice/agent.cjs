...
function createTTSPronunciationMapStream(text, pronunciationMap) {
  ...
  const cancel = async (reason) => {
    cancelled = true;
    await (reader == null ? void 0 : reader.cancel(reason));
  };
...
function drainTTSPronunciationBuffer(initialBuffer, entries, final = false) {
  ...
  while (buffer.length > 0) {
    ...
    const mayBecomeMatch = !final && entries.some(([term]) => term.length > 0 && term.startsWith(buffer));
    if (mayBecomeMatch) {
      break;
    }
    output += buffer[0];
