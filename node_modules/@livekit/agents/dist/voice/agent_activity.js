...
class AgentActivity {
  ...
  async _startSession(options) {
    ...
    if (this.llm instanceof RealtimeModel) {
      ...
      try {
        const realtimeInstructions = !rtReused || capabilities.midSessionInstructionsUpdate ? renderInstructions(this.agent.instructions) : void 0;
  ...
  async _detachReusableResources(newActivity) {
    ...
    try {
      ...
      if (this.realtimeSession && this.llm instanceof RealtimeModel && this.llm === newActivity.llm) {
        ...
        reusable = reusable && (capabilities.midSessionInstructionsUpdate || instructionsEqual(this.agent.instructions, newActivity.agent.instructions));
        reusable = reusable && (capabilities.midSessionToolsUpdate || isSameToolContext(this.tools, newActivity.tools));
        if (reusable) {
  ...
  attachAudioInput(audioStream) {
    ...
    const discardAudioFilter = new TransformStream({
  ...
  onPreemptiveGeneration(info) {
    ...
    if (!preemptiveOpts.enabled || this.schedulingPaused || this._currentSpeech !== void 0 && !this._currentSpeech.interrupted || !(this.llm instanceof LLM)) {
      return;
    }
    ...
    if (functionToolsExecutedEvent.functionCallOutputs.length > 0) {
      while (this.currentSpeech || this.speechQueue.size() > 0) {
        if (this.currentSpeech && !this.currentSpeech.done() && this.currentSpeech !== speechHandle) {
          ...
          await new ThrowsPromise((resolve) => setImmediate(resolve));
        }
      }
      ...
      if (this.llm.capabilities.autoToolReplyGeneration && shouldGenerateToolReply && this.pendingAutoToolReplyFut === void 0) {
        ...
        if (runState && !runState.done()) {
          ...
          const waitTask = Task.from(
            async () => {
              try {
                await waitUntilTimeout(fut.await, 5e3);
              } catch (error) {
  ...
  async realtimeReplyTask({
    ...
    try {
      const generationEvent = await this.realtimeSession.generateReply(
        instructions !== void 0 ? renderInstructions(instructions, speechHandle.inputDetails.modality) : void 0
      );
  ...
  async drain(options) {
    return tracer.startActiveSpan(async (span) => this._drainImpl(span, options == null ? void 0 : options.newActivity), {
