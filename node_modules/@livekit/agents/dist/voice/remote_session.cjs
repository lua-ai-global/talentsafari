    ...
    return {
      next: () => {
        if (this.closed && this.pendingMessages.length === 0) {
          return import_throws.ThrowsPromise.resolve({
            value: void 0,
            done: true
          });
        }
        ...
        if (pending) {
          return import_throws.ThrowsPromise.resolve({ value: pending, done: false });
        }
        return new import_throws.ThrowsPromise((resolve) => {
...
const AMD_CATEGORY_MAP = {
  [import_amd.AMDCategory.HUMAN]: import_protocol.AgentSession.AmdCategory.AMD_HUMAN,
  [import_amd.AMDCategory.MACHINE_IVR]: import_protocol.AgentSession.AmdCategory.AMD_MACHINE_IVR,
  [import_amd.AMDCategory.MACHINE_VM]: import_protocol.AgentSession.AmdCategory.AMD_MACHINE_VM,
  [import_amd.AMDCategory.MACHINE_UNAVAILABLE]: import_protocol.AgentSession.AmdCategory.AMD_MACHINE_UNAVAILABLE,
  [import_amd.AMDCategory.UNCERTAIN]: import_protocol.AgentSession.AmdCategory.AMD_UNCERTAIN
};
...
function msToDuration(ms) {
  const wholeSeconds = Math.trunc(ms / 1e3);
  const remainderMs = ms - wholeSeconds * 1e3;
  return new import_protobuf.Duration({
    seconds: BigInt(wholeSeconds),
    nanos: Math.round(remainderMs * 1e6)
  });
}
function chatItemsToProto(items) {
  return items.filter((item) => item.type !== "agent_config_update").map(chatItemToProto);
}
  ...
  switch (item.type) {
    case "message": {
      ...
      for (const c of msg.content) {
        ...
        } else if ((0, import_chat_context.isInstructions)(c)) {
          content.push(
            new import_protocol.AgentSession.ChatMessage_ChatContent({ payload: { case: "text", value: c.value } })
          );
        }
      }
...
function protoSerializeOptions(opts) {
  ...
  return {
    ...
    preemptive_generation: JSON.stringify(((_c = opts.turnHandling) == null ? void 0 : _c.preemptiveGeneration) ?? {}),
...
class SessionHost {
  ...
  async close() {
    ...
    await import_throws.ThrowsPromise.allSettled([...this.tasks].map((task) => task.cancelAndWait()));
    this.tasks.clear();
   ...
   */
  _onAmdPrediction(event) {
    this.emitEvent({
      case: "amdPrediction",
      value: new import_protocol.AgentSession.AgentSessionEvent_AmdPrediction({
  ...
  async handleRequest(req) {
    ...
    switch (req.request.case) {
      ...
      case "getFrameworkInfo":
        return this.sendResponse(req.requestId, {
          case: "getFrameworkInfo",
          value: new import_protocol.AgentSession.SessionResponse_GetFrameworkInfoResponse({
            sdk: "js",
  ...
  async handleGetAgentInfo(requestId) {
    ...
    return this.sendResponse(requestId, {
      ...
      value: new import_protocol.AgentSession.SessionResponse_GetAgentInfoResponse({
        id: agent.id,
        instructions: (0, import_chat_context.renderInstructions)(agent.instructions),
