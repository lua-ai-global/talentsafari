...
__export(agent_activity_exports, {
  ...
  SchedulingPausedError: () => SchedulingPausedError,
  agentActivityStorage: () => agentActivityStorage,
  cleanupReusableResources: () => cleanupReusableResources,
  isSchedulingPausedError: () => isSchedulingPausedError,
...
class AgentActivity {
  ...
  _backgroundSpeeches = /* @__PURE__ */ new Set();
  ...
  onInterruptionMetricsCollected = (ev) => {
    ...
    this.agentSession.emit(
      import_events.AgentSessionEventTypes.SessionUsageUpdated,
      (0, import_events.createSessionUsageUpdatedEvent)({ usage })
    );
  };
  ...
  async start(options) {
    ...
    try {
      await this._startSession({
        ...
        runOnEnter: true,
        reuseResources: options == null ? void 0 : options.reuseResources
      });
    } finally {
      unlock();
    }
  ...
  async _startSession(options) {
    ...
    if (this.llm instanceof import_llm.RealtimeModel) {
      ...
      try {
        const realtimeInstructions = !rtReused || capabilities.midSessionInstructionsUpdate ? (0, import_chat_context.renderInstructions)(this.agent.instructions) : void 0;
        await this.realtimeSession._updateSession(
          realtimeInstructions,
          !rtReused || capabilities.midSessionChatCtxUpdate ? this.agent.chatCtx : void 0,
          !rtReused || capabilities.midSessionToolsUpdate ? this.tools : void 0
        );
      } catch (error) {
    ...
    if (runOnEnter && (this.agent.instructions || initialTools.length > 0)) {
      const initialConfig = new import_chat_context.AgentConfigUpdate({
        instructions: this.agent.instructions,
        toolsAdded: initialTools.length > 0 ? initialTools : void 0
      });
    ...
    this.audioRecognition = new import_audio_recognition.AudioRecognition({
      ...
      backchannelBoundary: this.agentSession.sessionOptions.turnHandling.interruption.backchannelBoundary,
      endpointing: (0, import_endpointing.createEndpointing)({
        ...this.agentSession.sessionOptions.turnHandling.endpointing,
        ...((_a = this.agent.turnHandling) == null ? void 0 : _a.endpointing) ?? {}
      }),
      rootSpanContext: this.agentSession.rootSpanContext,
      sttModel: (_b = this.stt) == null ? void 0 : _b.label,
    ...
    if (reuseResources == null ? void 0 : reuseResources.sttPipeline) {
      ...
      reuseResources.sttPipeline = void 0;
    } else {
  ...
  async _detachReusableResources(newActivity) {
    const resources = {};
    try {
      if (this.audioRecognition && this.stt && newActivity.stt && this.stt === newActivity.stt && Object.getPrototypeOf(this.agent).sttNode === Object.getPrototypeOf(newActivity.agent).sttNode) {
        resources.sttPipeline = await this.audioRecognition.detachSttPipeline();
      }
      if (this.realtimeSession && this.llm instanceof import_llm.RealtimeModel && this.llm === newActivity.llm) {
        const capabilities = this.llm.capabilities;
        let reusable = capabilities.midSessionChatCtxUpdate || this.realtimeSession.chatCtx.copy({ excludeInstructions: true, excludeHandoff: true, excludeConfigUpdate: true }).isEquivalent(
        ...
        reusable = reusable && (capabilities.midSessionInstructionsUpdate || (0, import_chat_context.instructionsEqual)(this.agent.instructions, newActivity.agent.instructions));
        reusable = reusable && (capabilities.midSessionToolsUpdate || (0, import_tool_context.isSameToolContext)(this.tools, newActivity.tools));
        if (reusable) {
  ...
  subscribeAudioStream() {
    var _a;
    return (_a = this.audioRecognition) == null ? void 0 : _a.subscribeAudioStream();
  }
  ...
  async updateTools(tools) {
    ...
    if (toolsAdded.length > 0 || toolsRemoved.length > 0) {
      const configUpdate = new import_chat_context.AgentConfigUpdate({
        toolsAdded: toolsAdded.length > 0 ? toolsAdded : void 0,
        toolsRemoved: toolsRemoved.length > 0 ? toolsRemoved : void 0
      });
  ...
  attachAudioInput(audioStream) {
    ...
    const discardAudioFilter = new import_web.TransformStream({
  ...
  shouldDiscardInputAudio() {
    var _a, _b;
    const aecWarmupActive = this.agentSession.agentState === "speaking" && this.agentSession._aecWarmupRemaining > 0;
    const discardAudioIfUninterruptible = ((_b = (_a = this.agent.turnHandling) == null ? void 0 : _a.interruption) == null ? void 0 : _b.discardAudioIfUninterruptible) ?? this.agentSession.sessionOptions.turnHandling.interruption.discardAudioIfUninterruptible;
    const uninterruptibleSpeechActive = this._currentSpeech !== void 0 && !this._currentSpeech.done() && !this._currentSpeech.interrupted && !this._currentSpeech.allowInterruptions && discardAudioIfUninterruptible;
  ...
  onGenerationCreated(ev) {
    ...
    if (fut && !fut.done) {
      ...
      this.pendingAutoToolReplyFut = void 0;
      fut.resolve();
    }
    this.scheduleSpeech(handle, import_speech_handle.SpeechHandle.SPEECH_PRIORITY_NORMAL);
  }
  ...
  onStartOfSpeech(ev) {
    ...
    if (this.falseInterruptionTimer) {
      ...
      this.falseInterruptionTimer = void 0;
    }
    if (this.agentSession.agentState !== "speaking" && this.pauseEnabled() && this._currentSpeech !== void 0 && !this._currentSpeech.interrupted && this._currentSpeech.allowInterruptions && (!this.pausedSpeech || this.pausedSpeech.handle !== this._currentSpeech)) {
  ...
  interruptByAudioActivity(options) {
    var _a, _b, _c, _d;
    ...
    if (this._currentSpeech && !this._currentSpeech.interrupted && this._currentSpeech.allowInterruptions) {
      ...
      if (this.pauseEnabled()) {
        ...
        const audioOutput = this.agentSession.output.audio;
        if (this.isInterruptionDetectionEnabled && this.audioRecognition && this.agentSession.agentState === "speaking") {
          this.audioRecognition.onStartOfOverlapSpeech(
            0,
            Date.now(),
            this.agentSession._userSpeakingSpan
        ...
        if (this.audioRecognition) {
          this.audioRecognition.onEndOfAgentSpeech(
            (options == null ? void 0 : options.ignoreUserTranscriptUntil) ?? Date.now()
          );
        }
  ...
  onInterimTranscript(ev, speaking) {
    ...
    if (ev.alternatives[0].text && this.turnDetection !== "manual" && this.turnDetection !== "realtime_llm") {
      this.interruptByAudioActivity();
      if (speaking === false && this.pausedSpeech && this.agentSession.sessionOptions.turnHandling.interruption.falseInterruptionTimeout !== void 0) {
  ...
  onPreemptiveGeneration(info) {
    ...
    if (!preemptiveOpts.enabled || this.schedulingPaused || this._currentSpeech !== void 0 && !this._currentSpeech.interrupted || !(this.llm instanceof import_llm.LLM)) {
      return;
    }
    this.cancelPreemptiveGeneration();
    if (info.startedSpeakingAt !== void 0 && Date.now() - info.startedSpeakingAt > preemptiveOpts.maxSpeechDuration) {
      return;
    }
  ...
  _interruptBackgroundSpeeches(force) {
    const interrupted = [];
  ...
  async mainTask(signal) {
    ...
    while (true) {
      await import_throws.ThrowsPromise.race([this.q_updated.await, abortFuture.await]);
  ...
  generateReply(options) {
    ...
    if (this.llm instanceof import_llm.RealtimeModel) {
      ...
      if (instructions) {
        instructions = (0, import_chat_context.concatInstructions)(this.agent.instructions, "\n", instructions);
      }
  ...
  async ttsTask(speechHandle, text, addToChatCtx, modelSettings, replyAbortController, audio) {
    ...
    let replyStartedForwardingAt;
    let replyTtsGenData = null;
    const onFirstFrame = (audioOut, startedSpeakingAt = Date.now()) => {
      replyStartedSpeakingAt = startedSpeakingAt;
      replyStartedForwardingAt = (audioOut == null ? void 0 : audioOut.startedForwardingAt) ?? replyStartedSpeakingAt;
    ...
    if (!audioOutput) {
      if (textOut) {
        textOut.firstTextFut.await.then(() => onFirstFrame(null)).catch(() => this.logger.debug("firstTextFut cancelled before first frame"));
      }
    } else {
      let audioOut = null;
      if (!audio) {
      ...
      audioOut.firstFrameFut.await.then((ts) => onFirstFrame(audioOutForCb, ts)).catch(() => this.logger.debug("firstFrameFut cancelled before first frame"));
    }
    ...
    if (addToChatCtx) {
      ...
      if (replyStartedSpeakingAt !== void 0) {
        ...
        if (replyStartedForwardingAt !== void 0) {
          replyAssistantMetrics.playbackLatency = (replyStartedSpeakingAt - replyStartedForwardingAt) / 1e3;
        }
      }
      const message = import_chat_context.ChatMessage.create({
        role: "assistant",
  ...
  _pipelineReplyTaskImpl = async ({
    ...
    if (instructions) {
      span.setAttribute(import_telemetry.traceTypes.ATTR_INSTRUCTIONS, (0, import_chat_context.renderInstructions)(instructions));
    }
    if (newMessage) {
    ...
    (0, import_generation.applyInstructionsModality)(chatCtx, { modality: speechHandle.inputDetails.modality });
    const tasks = [];
    ...
    const startTtsInference = () => {
      var _a2, _b2;
      return (0, import_generation.performTTSInference)(
        (...args) => this.agent.ttsNode(...args),
        ttsTextInput,
        modelSettings,
        replyAbortController,
        (_a2 = this.tts) == null ? void 0 : _a2.model,
        (_b2 = this.tts) == null ? void 0 : _b2.provider,
        this.agentSession.sessionOptions.ttsReadIdleTimeout,
        this.agentSession.sessionOptions.ttsTextTransforms
      );
    };
    ...
    if (this.useTtsAlignedTranscript && ((_d = this.tts) == null ? void 0 : _d.capabilities.alignedTranscript) && ttsGenData) {
      const timedTextsStream = await import_throws.ThrowsPromise.race([
        ...
        )) ?? import_throws.ThrowsPromise.resolve()
      ]);
      if (timedTextsStream) {
    ...
    let agentStartedForwardingAt;
    const onFirstFrame = (audioOutRef, startedSpeakingAt = Date.now()) => {
      agentStartedSpeakingAt = startedSpeakingAt;
      agentStartedForwardingAt = (audioOutRef == null ? void 0 : audioOutRef.startedForwardingAt) ?? agentStartedSpeakingAt;
      this.agentSession._updateAgentState("speaking", {
        startTime: startedSpeakingAt,
        otelContext: speechHandle._agentTurnContext
      });
    ...
    if (audioOutput) {
      if (ttsGenData) {
        ...
        audioOut = _audioOut;
        tasks.push(forwardTask);
        audioOut.firstFrameFut.await.then((ts) => onFirstFrame(audioOut, ts)).catch(() => this.logger.debug("firstFrameFut cancelled before first frame"));
      } else {
        throw Error("ttsGenData is null when audioOutput is enabled");
      }
    } else {
      textOut == null ? void 0 : textOut.firstTextFut.await.then(() => onFirstFrame(null)).catch(() => this.logger.debug("firstTextFut cancelled before first frame"));
    }
    ...
    if (agentStartedSpeakingAt !== void 0) {
      ...
      if (agentStartedForwardingAt !== void 0) {
        assistantMetrics.playbackLatency = (agentStartedSpeakingAt - agentStartedForwardingAt) / 1e3;
      }
    ...
    if (speechHandle.interrupted) {
      ...
      if (forwardedText) {
        ...
        const message = import_chat_context.ChatMessage.create({
          ...
          ...Object.keys(llmGenData.generatedExtra).length > 0 ? { extra: llmGenData.generatedExtra } : {}
        });
  ...
  async realtimeGenerationTask(speechHandle, ev, modelSettings, replyAbortController, addToChatCtx = true) {
  ...
  async _realtimeGenerationTaskImpl({
    ...
    const onFirstFrame = (startedSpeakingAt = Date.now()) => {
      this.agentSession._updateAgentState("speaking", {
        startTime: startedSpeakingAt,
        otelContext: speechHandle._agentTurnContext
      });
    ...
    if (functionToolsExecutedEvent.functionCallOutputs.length > 0) {
      while (this.currentSpeech || this.speechQueue.size() > 0) {
        if (this.currentSpeech && !this.currentSpeech.done() && this.currentSpeech !== speechHandle) {
          ...
          await new import_throws.ThrowsPromise((resolve) => setImmediate(resolve));
        }
      }
      ...
      let fut;
      if (this.llm.capabilities.autoToolReplyGeneration && shouldGenerateToolReply && this.pendingAutoToolReplyFut === void 0) {
        const runState = this.agentSession._globalRunState;
        if (runState && !runState.done()) {
          fut = new import_utils.Future();
          this.pendingAutoToolReplyFut = fut;
          const llmLabel = this.llm.label();
          const waitTask = import_utils.Task.from(
            async () => {
              try {
                await (0, import_utils.waitUntilTimeout)(fut.await, 5e3);
              } catch (error) {
                if (error instanceof import_utils.IdleTimeoutError) {
                  this.logger.warn(
                    { llm: llmLabel },
  ...
  async realtimeReplyTask({
    ...
    try {
      const generationEvent = await this.realtimeSession.generateReply(
        instructions !== void 0 ? (0, import_chat_context.renderInstructions)(instructions, speechHandle.inputDetails.modality) : void 0
      );
  ...
  async drain(options) {
    return import_telemetry.tracer.startActiveSpan(async (span) => this._drainImpl(span, options == null ? void 0 : options.newActivity), {
      name: "drain_agent_activity",
      context: import_api.ROOT_CONTEXT
    });
  }
  async _drainImpl(span, newActivity) {
    span.setAttribute(import_telemetry.traceTypes.ATTR_AGENT_LABEL, this.agent.id);
    const unlock = await this.lock.lock();
    try {
      if (this._schedulingPaused) return void 0;
  ...
  async close() {
    ...
    try {
      ...
      this.cancelSpeechPauseTask = void 0;
      await this._closeSessionResources();
      if (this._mainTask) {
  ...
  updatePausedSpeech(speechHandle, timeout) {
  ...
  pauseEnabled() {
    ...
    return !!(interruptionOptions.resumeFalseInterruption && interruptionOptions.falseInterruptionTimeout !== void 0 && this.agentSession.output.audio && this.agentSession.output.audio.canPause);
  }
  startFalseInterruptionTimer(timeout) {
    if (this.falseInterruptionTimer !== void 0) {
      clearTimeout(this.falseInterruptionTimer);
    }
    this.falseInterruptionTimer = setTimeout(() => {
      if (!this.pausedSpeech || this._currentSpeech && this._currentSpeech !== this.pausedSpeech.handle) {
        this.pausedSpeech = void 0;
        return;
      }
      let resumed = false;
      const interruptionOptions = this.agentSession.sessionOptions.turnHandling.interruption;
      const audioOutput = this.agentSession.output.audio;
      if (interruptionOptions.resumeFalseInterruption && audioOutput && audioOutput.canPause && !this.pausedSpeech.handle.done()) {
      ...
      this.agentSession.emit(
        import_events.AgentSessionEventTypes.AgentFalseInterruption,
        (0, import_events.createAgentFalseInterruptionEvent)({ resumed })
      );
      this.pausedSpeech = void 0;
      this.falseInterruptionTimer = void 0;
    }, timeout);
  }
  async cancelSpeechPause(options) {
    ...
    if (interrupt && !this.pausedSpeech.handle.interrupted && this.pausedSpeech.handle.allowInterruptions) {
  ...
  disableVadInterruptionSoon() {
    ...
    if (audioRecognition && audioRecognition.backchannelBoundaryActive) {
      audioRecognition.backchannelBoundaryCallback = () => {
        if (this.agentSession.agentState === "speaking" && this.isInterruptionByAudioActivityEnabled) {
  ...
  restoreInterruptionByAudioActivity() {
    var _a;
    (_a = this.audioRecognition) == null ? void 0 : _a.cancelBackchannelBoundary();
    this.isInterruptionByAudioActivityEnabled = this.isDefaultInterruptionByAudioActivityEnabled;
  }
  fallbackToVadInterruption(error) {
    ...
    this.logger.info(
      {
        ...
        label: error == null ? void 0 : error.label
      },
