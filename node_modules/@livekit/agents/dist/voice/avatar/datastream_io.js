...
class DataStreamAudioOutput extends AudioOutput {
  ...
  static _playbackStartedRpcRegistered = false;
  static _playbackStartedHandlers = {};
  room;
  destinationIdentity;
  roomConnectedFuture;
  waitRemoteTrack;
  ...
  static registerPlaybackStartedRpc({
    ...
    const rpcHandler = async (data) => {
      const handler2 = DataStreamAudioOutput._playbackStartedHandlers[data.callerIdentity];
      if (!handler2) {
        log().warn(
          {
            callerIdentity: data.callerIdentity,
    ...
    (_a = room.localParticipant) == null ? void 0 : _a.registerRpcMethod(RPC_PLAYBACK_STARTED, rpcHandler);
