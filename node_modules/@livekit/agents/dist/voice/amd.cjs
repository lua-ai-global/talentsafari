...
var amd_exports = {};
__export(amd_exports, {
  AMD: () => AMD,
  AMDCategory: () => AMDCategory
});
module.exports = __toCommonJS(amd_exports);
...
class AMD extends import_node_events.EventEmitter {
  constructor(session, options = {}) {
    ...
    if (llm === void 0 || stt === void 0) {
      ...
      try {
        autoSelect = (0, import_utils.isCloud)(new URL(rawUrl)) && !!(apiKey && apiSecret);
      } catch {
      }
  ...
  async execute() {
    return import_telemetry.tracer.startActiveSpan(
      async (span) => {
        ...
        span.setAttribute(import_telemetry.traceTypes.ATTR_AMD_INTERRUPT_ON_MACHINE, this.interruptOnMachine);
        span.setAttribute(import_telemetry.traceTypes.ATTR_GEN_AI_OPERATION_NAME, "classification");
        const linkedParticipant = (_a = this.session._roomIO) == null ? void 0 : _a.linkedParticipant;
        if (linkedParticipant) {
          (0, import_utils2.setParticipantSpanAttributes)(span, linkedParticipant);
        }
        try {
  ...
  subscribe() {
    this.session.on(import_events.AgentSessionEventTypes.UserInputTranscribed, this.handleTranscript);
    this.session.on(import_events.AgentSessionEventTypes.UserStateChanged, this.handleUserStateChanged);
    this.session.on(import_events.AgentSessionEventTypes.Close, this.handleClose);
  }
  /**
  ...
  startSTTPump() {
    if (!this.stt) return;
    this.sttPumpTask = import_utils.Task.from(({ signal }) => this.runSTTPump(signal));
  ...
  async runSTTPump(signal) {
    ...
    while (!signal.aborted && !this.settled) {
      ...
      try {
        await (0, import_utils.delay)(100, { signal });
      } catch {
        return;
      }
    }
    ...
    const recvPump = (async () => {
      ...
      try {
        for await (const event of sttStream) {
          ...
          if (event.type !== import_stt.SpeechEventType.FINAL_TRANSCRIPT) continue;
  ...
  gateNoSpeechTimer() {
    ...
    (0, import_utils.waitForTrackPublication)({
      room,
      identity: targetIdentity,
      kind: import_rtc_node.TrackKind.KIND_AUDIO,
  ...
  cleanup() {
    ...
    this.session.off(import_events.AgentSessionEventTypes.UserInputTranscribed, this.handleTranscript);
    this.session.off(import_events.AgentSessionEventTypes.UserStateChanged, this.handleUserStateChanged);
    this.session.off(import_events.AgentSessionEventTypes.Close, this.handleClose);
   ...
   */
  resolveLLM(option) {
    if (option instanceof import_llm.LLM) {
   ...
   */
  resolveSTT(option) {
    if (option instanceof import_stt.STT) {
  ...
  setSpanAttributes(result) {
    ...
    (_a = this.span) == null ? void 0 : _a.setAttribute(import_telemetry.traceTypes.ATTR_AMD_CATEGORY, result.category);
    (_b = this.span) == null ? void 0 : _b.setAttribute(import_telemetry.traceTypes.ATTR_AMD_REASON, result.reason);
    (_c = this.span) == null ? void 0 : _c.setAttribute(import_telemetry.traceTypes.ATTR_AMD_IS_MACHINE, result.isMachine);
    (_d = this.span) == null ? void 0 : _d.setAttribute(import_telemetry.traceTypes.ATTR_AMD_SPEECH_DURATION, result.speechDurationMs);
    (_e = this.span) == null ? void 0 : _e.setAttribute(import_telemetry.traceTypes.ATTR_AMD_DELAY, result.delayMs);
    (_f = this.span) == null ? void 0 : _f.setAttribute(import_telemetry.traceTypes.ATTR_AMD_TRANSCRIPT, result.transcript);
  }
  /**
  ...
  async detect(transcript, generation) {
    ...
    const savePrediction = (0, import_tool_context.tool)({
      description: "Save the AMD prediction to the verdict.",
      parameters: import_zod.z.object({
        label: import_zod.z.enum([
          "human" /* HUMAN */,
          "machine-ivr" /* MACHINE_IVR */,
          "machine-vm" /* MACHINE_VM */,
          "machine-unavailable" /* MACHINE_UNAVAILABLE */,
    ...
    const postponeTermination = (0, import_tool_context.tool)({
      ...
      parameters: import_zod.z.object({
        seconds: import_zod.z.number().describe("Additional seconds to wait (max 10).")
      }),
...
Classify this call answer.`
    ...
    for (const tc of toolCalls) {
      ...
      if (!fnTool || !(0, import_tool_context.isFunctionTool)(fnTool)) continue;
      let parsedArgs = {};
      try {
