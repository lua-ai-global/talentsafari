...
class AgentSession extends import_node_events.EventEmitter {
  ...
  async _startImpl({
    ...
    await import_throws.ThrowsPromise.allSettled(tasks);
    if (this.sessionHost) {
    ...
    if (this.sessionOptions.turnHandling.interruption.resumeFalseInterruption && this.output.audio && !this.output.audio.canPause) {
      this.logger.warn(
        {
  ...
  async start({
    ...
    this.rootSpanContext = import_api.trace.setSpan(import_api.context.active(), this.sessionSpan);
  ...
  _onAmdPrediction(event) {
    var _a;
    (_a = this.sessionHost) == null ? void 0 : _a._onAmdPrediction(event);
  }
  /**
   ...
   */
  _subscribeAudioStream() {
    var _a;
    return (_a = this.activity) == null ? void 0 : _a.subscribeAudioStream();
  }
  ...
  generateReply(options) {
    ...
    const inputDetails = { modality: (options == null ? void 0 : options.inputModality) ?? "text" };
    const doGenerateReply = (activity, nextActivity) => {
      ...
      try {
        ...
        const canFallback = nextActivity !== void 0 && (0, import_agent_activity.isSchedulingPausedError)(error);
        if (!canFallback) {
          throw error;
        }
        this.logger.debug(
  ...
  run({
    ...
    (async () => {
      try {
        ...
        runState._reject((0, import_utils.asError)(e));
      }
    })();
    return runState;
  }
    ...
    const runWithContext = async () => {
      ...
      try {
        ...
        if (this.closing && newActivity === "start") {
          this.logger.warn(
            { agentId: (_a = this.nextActivity) == null ? void 0 : _a.agent.id },
            "Session is closing, skipping start of next activity"
          );
          if (reusableResources) {
            await (0, import_agent_activity.cleanupReusableResources)(reusableResources, this.logger);
            reusableResources = void 0;
          }
  ...
  onAudioOutputChanged() {
    if (this.started && this.sessionOptions.turnHandling.interruption.resumeFalseInterruption && this.output.audio && !this.output.audio.canPause) {
      this.logger.warn(
        {
          audioOutput: this.output.audio.constructor.name
        },
  ...
  async closeImplInner(reason, error = null, drain = false) {
    ...
    await ((_b = this.activity) == null ? void 0 : _b.close());
    this.activity = void 0;
    if (this.sessionSpan) {
    ...
    this.llmErrorCounts = 0;
    this.ttsErrorCounts = 0;
    await ((_c = this.sessionHost) == null ? void 0 : _c.close());
    this.sessionHost = void 0;
    await ((_d = this._roomIO) == null ? void 0 : _d.close());
    this._roomIO = void 0;
