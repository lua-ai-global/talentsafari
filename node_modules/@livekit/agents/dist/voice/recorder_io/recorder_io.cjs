...
class RecorderIO {
  ...
  async close() {
    ...
    try {
      ...
      this.started = false;
      if (this.forwardTask) {
        await (0, import_utils.cancelAndWait)([this.forwardTask]);
      }
      await this.inChan.close();
      await this.outChan.close();
      await this.closeFuture.await;
      if (this.encodeTask) {
        await (0, import_utils.cancelAndWait)([this.encodeTask]);
      }
      await ((_a = this.inRecord) == null ? void 0 : _a.close());
      this.closing = false;
    } finally {
      unlock();
    }
  ...
  writeCb(buf) {
    ...
    this.inChan.write(inputBuf).catch((err) => {
      if (!(0, import_utils.isWritableStreamClosedError)(err)) {
  ...
  async forward(signal) {
    while (!signal.aborted && this.started && !this.closing) {
      ...
      try {
        ...
        if (this.inChan.closed || this.outChan.closed || (0, import_utils.isWritableStreamClosedError)(err)) {
          break;
        }
        this.logger.error({ err }, "Error writing RecorderIO output buffer");
      }
  ...
  async encode() {
    ...
    try {
      ...
      (_a = this.inResampler) == null ? void 0 : _a.close();
      (_b = this.outResampler) == null ? void 0 : _b.close();
  ...
  onPlaybackFinished(options) {
    ...
    if (filteredBuf.length > 0) {
      if (trailingSilenceDuration > 0) {
        filteredBuf.push(
          createSilenceFrame(trailingSilenceDuration / 1e3, sampleRate, numChannels)
        );
      }
