  ...
  if (item.type === "message") {
    const msg = {
      ...
      content: item.content.map((c) => ({
        text: (0, import_chat_context.isInstructions)(c) ? c.value : c
      })),
    ...
    if (metrics && Object.keys(metrics).length > 0) {
      ...
      if (metrics.playbackLatency !== void 0) {
    ...
    if (item.instructions !== void 0) {
      configUpdate.instructions = (0, import_chat_context.renderInstructions)(item.instructions);
    }
    if (item.toolsAdded !== void 0) {
      configUpdate.toolsAdded = item.toolsAdded;
    }
    if (item.toolsRemoved !== void 0) {
