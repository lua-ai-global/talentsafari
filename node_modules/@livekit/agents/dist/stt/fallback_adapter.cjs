...
class FallbackAdapter extends import_stt.STT {
  sttInstances;
  attemptTimeoutMs;
  maxRetryPerSTT;
  retryIntervalMs;
  ...
  constructor(opts) {
    ...
    const wrapped = opts.sttInstances.map(
      (s) => s.capabilities.streaming ? s : new import_stream_adapter.StreamAdapter(s, opts.vad)
    );
  ...
  tryRecoverRecognize(stt, frame) {
    ...
    status.recoveringRecognizeTask = import_utils.Task.from(async (controller) => {
      try {
        ...
        if (e instanceof import_exceptions.APIError) {
  ...
  async close() {
    ...
    if (tasks.length > 0) {
      await (0, import_utils.cancelAndWait)(tasks, 1e3);
    }
    for (const s of this.sttInstances) {
...
class FallbackSpeechStream extends import_stt.SpeechStream {
  ...
  tryRecoverStream(sttInstance) {
    ...
    status.recoveringStreamTask = import_utils.Task.from(async (controller) => {
      var _a, _b;
      try {
  ...
  async run() {
    ...
    const forwarderTask = import_utils.Task.from(async (controller) => {
    ...
    if (!forwarderTask.done) {
      await (0, import_utils.cancelAndWait)([forwarderTask], 1e3);
    }
    ...
    if (liveProbeTasks.length > 0) {
      await (0, import_utils.cancelAndWait)(liveProbeTasks, 1e3);
    }
    for (const probe of [...this.recoveringStreams]) {
      try {
        probe.close();
