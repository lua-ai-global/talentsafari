...
var fake_stt_exports = {};
__export(fake_stt_exports, {
  FakeRecognizeStream: () => FakeRecognizeStream,
  FakeSTT: () => FakeSTT,
  RecognizeSentinel: () => RecognizeSentinel,
  emptyAudioFrame: () => emptyAudioFrame,
  speedUpFakeUserSpeech: () => speedUpFakeUserSpeech
});
module.exports = __toCommonJS(fake_stt_exports);
...
class FakeSTT extends import_stt.STT {
  ...
  _fakeUserSpeeches;
  _fakeRequireAudio;
  _recognizeCh = new import_utils.AsyncIterableQueue();
  _streamCh = new import_utils.AsyncIterableQueue();
  ...
  async _recognize(_frame) {
    if (this._fakeTimeoutMs !== null) {
      await (0, import_utils.delay)(this._fakeTimeoutMs);
    }
      ...
      alternatives: [
        {
          ...
          language: (0, import_language.asLanguageCode)(""),
          startTime: 0,
          endTime: 0,
          confidence: 1
        }
...
class FakeRecognizeStream extends import_stt.SpeechStream {
  label;
  _attempt = 0;
  _fakeStt;
  ...
  sendFakeTranscript(transcript, isFinal = true) {
    this.queue.put({
      type: isFinal ? import_stt.SpeechEventType.FINAL_TRANSCRIPT : import_stt.SpeechEventType.INTERIM_TRANSCRIPT,
      alternatives: [
        {
          text: transcript,
          language: (0, import_language.asLanguageCode)(""),
  ...
  async run() {
    ...
    if (state.fakeTimeoutMs !== null) {
      await (0, import_utils.delay)(state.fakeTimeoutMs);
    }
    if (state.fakeRequireAudio) {
      let gotAudio = false;
      for await (const data of this.input) {
        if (data === import_stt.SpeechStream.FLUSH_SENTINEL) {
  ...
  async fakeUserSpeechTask() {
    ...
    for (const speech of speeches) {
      ...
      if (elapsed() < interimAt) await (0, import_utils.delay)(interimAt - elapsed());
      ...
      if (elapsed() < finalAt) await (0, import_utils.delay)(finalAt - elapsed());
...
//# sourceMappingURL=fake_stt.cjs.map