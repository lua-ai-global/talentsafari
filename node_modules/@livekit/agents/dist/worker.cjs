...
class Default {
  ...
  static numIdleProcesses(production) {
    if (production) {
      return Math.min((0, import_node_os.availableParallelism)(), 4);
    } else {
      return 0;
    }
  }
...
class PendingAssignment {
  promise = new import_throws.ThrowsPromise((resolve) => {
...
class AgentServer {
  ...
  async run() {
    ...
    const workerWS = async () => {
      ...
      while (!this.#closed) {
        ...
        try {
          await new import_throws.ThrowsPromise((resolve, reject) => {
          ...
          await new import_throws.ThrowsPromise((resolve) => setTimeout(resolve, delay * 1e3));
        }
      }
    };
    await import_throws.ThrowsPromise.all([workerWS(), this.#httpServer.run()]);
    this.#close.resolve();
  }
  get id() {
    return this.#id;
  ...
  async drain(timeout) {
    ...
    const joinJobs = async () => {
      return import_throws.ThrowsPromise.all(
    ...
    if (timeout) {
      promises.push(
        (0, import_utils.rejectOnAbort)(AbortSignal.timeout(timeout)).catch(() => {
          throw new WorkerError("timed out draining");
        })
      );
    }
    await import_throws.ThrowsPromise.race(promises);
  }
  ...
  async #runWS(ws) {
    ...
    const close = new import_throws.ThrowsPromise((resolve) => {
    ...
    const loadMonitor = setInterval(() => {
      ...
      if (this.#draining) {
        if (currentStatus !== import_protocol.WorkerStatus.WS_FULL) {
          currentStatus = import_protocol.WorkerStatus.WS_FULL;
          this.event.emit(
            "worker_msg",
            new import_protocol.WorkerMessage({
              message: {
  ...
  async #availability(msg) {
    ...
    if (this.#draining) {
      this.#logger.child({ jobId: (_b = msg.job) == null ? void 0 : _b.id, resuming: msg.resuming, agentName: this.#opts.agentName }).info("Worker is draining and no longer available, rejecting job");
      await req.reject();
      return;
    }
  ...
  async close() {
    ...
    await import_throws.ThrowsPromise.allSettled(this.#tasks);
