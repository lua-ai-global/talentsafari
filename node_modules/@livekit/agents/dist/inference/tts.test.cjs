...
(0, import_vitest.describe)("hasAlignedTranscript", () => {
  (0, import_vitest.it)("returns false for unknown provider", () => {
    (0, import_vitest.expect)((0, import_tts.hasAlignedTranscript)("rime/mistv2", { add_timestamps: true })).toBe(false);
    (0, import_vitest.expect)((0, import_tts.hasAlignedTranscript)("deepgram/aura-2", { sync_alignment: true })).toBe(false);
  });
  (0, import_vitest.it)("returns false for an empty options payload", () => {
    (0, import_vitest.expect)((0, import_tts.hasAlignedTranscript)("cartesia/sonic", {})).toBe(false);
    (0, import_vitest.expect)((0, import_tts.hasAlignedTranscript)("elevenlabs/eleven_flash_v2", void 0)).toBe(false);
    (0, import_vitest.expect)((0, import_tts.hasAlignedTranscript)(void 0, { add_timestamps: true })).toBe(false);
  });
  (0, import_vitest.it)("detects Cartesia add_timestamps opt-in", () => {
    (0, import_vitest.expect)((0, import_tts.hasAlignedTranscript)("cartesia/sonic", { add_timestamps: true })).toBe(true);
    (0, import_vitest.expect)((0, import_tts.hasAlignedTranscript)("cartesia/sonic-3", { add_timestamps: false })).toBe(false);
  });
  (0, import_vitest.it)("detects ElevenLabs sync_alignment opt-in", () => {
    (0, import_vitest.expect)((0, import_tts.hasAlignedTranscript)("elevenlabs/eleven_flash_v2", { sync_alignment: true })).toBe(true);
    (0, import_vitest.expect)(
      (0, import_tts.hasAlignedTranscript)("elevenlabs/eleven_multilingual_v2", { sync_alignment: false })
    ).toBe(false);
  });
  (0, import_vitest.it)("detects Inworld WORD/CHARACTER timestamp types", () => {
    (0, import_vitest.expect)((0, import_tts.hasAlignedTranscript)("inworld/inworld-tts-1", { timestamp_type: "WORD" })).toBe(true);
    (0, import_vitest.expect)((0, import_tts.hasAlignedTranscript)("inworld/inworld-tts-1", { timestamp_type: "CHARACTER" })).toBe(
      true
    );
    (0, import_vitest.expect)(
      (0, import_tts.hasAlignedTranscript)("inworld/inworld-tts-1", {
        timestamp_type: "TIMESTAMP_TYPE_UNSPECIFIED"
      })
    ).toBe(false);
  });
});
(0, import_vitest.describe)("TTS alignedTranscript capability", () => {
  (0, import_vitest.it)("defaults to alignedTranscript=false when no opt-in is provided", () => {
    const tts = makeTts();
    (0, import_vitest.expect)(tts.capabilities.alignedTranscript).toBe(false);
  });
  (0, import_vitest.it)("reports alignedTranscript=true when Cartesia add_timestamps is set", () => {
    const tts = makeTts({
      model: "cartesia/sonic",
      modelOptions: { add_timestamps: true }
    });
    (0, import_vitest.expect)(tts.capabilities.alignedTranscript).toBe(true);
  });
  (0, import_vitest.it)("reports alignedTranscript=true when ElevenLabs sync_alignment is set", () => {
    const tts = makeTts({
  ...
  (0, import_vitest.it)("reports alignedTranscript=true when Inworld timestamp_type is WORD", () => {
    const tts = makeTts({
      model: "inworld/inworld-tts-1",
  ...
  (0, import_vitest.it)("recomputes alignedTranscript when updateOptions changes modelOptions", () => {
  ...
  (0, import_vitest.it)("recomputes alignedTranscript when updateOptions changes the model", () => {
    const tts = makeTts({
      model: "cartesia/sonic",
      modelOptions: { sync_alignment: true }
    });
  ...
  (0, import_vitest.it)("invalidates the connection pool when session-affecting options change", () => {
    const tts = makeTts({ model: "cartesia/sonic" });
    const invalidateSpy = import_vitest.vi.spyOn(tts.pool, "invalidate");
    tts.updateOptions({ modelOptions: { add_timestamps: true } });
    (0, import_vitest.expect)(invalidateSpy).toHaveBeenCalledTimes(1);
    tts.updateOptions({ model: "elevenlabs/eleven_flash_v2" });
    (0, import_vitest.expect)(invalidateSpy).toHaveBeenCalledTimes(2);
    tts.updateOptions({ voice: "narrator" });
    (0, import_vitest.expect)(invalidateSpy).toHaveBeenCalledTimes(3);
    tts.updateOptions({ language: "en" });
    (0, import_vitest.expect)(invalidateSpy).toHaveBeenCalledTimes(4);
    tts.updateOptions({});
    (0, import_vitest.expect)(invalidateSpy).toHaveBeenCalledTimes(4);
  });
});
