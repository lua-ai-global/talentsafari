...
__export(tts_exports, {
  ...
  TTS: () => TTS,
  hasAlignedTranscript: () => hasAlignedTranscript,
  normalizeTTSFallback: () => normalizeTTSFallback,
  parseTTSModelString: () => parseTTSModelString
});
...
function parseTTSModelString(model) {
...
function hasAlignedTranscript(model, modelOptions) {
...
function normalizeTTSFallback(fallback) {
...
class TTS extends import_tts.TTS {
  ...
  constructor(opts) {
    ...
    const initialAligned = hasAlignedTranscript(
      opts == null ? void 0 : opts.model,
      opts == null ? void 0 : opts.modelOptions
    );
    ...
    const normalizedFallback = fallback ? normalizeTTSFallback(fallback) : void 0;
    this.opts = {
      model: nextModel,
      voice: nextVoice,
  ...
  updateOptions(opts) {
    const mergedModelOptions = opts.modelOptions !== void 0 ? { ...this.opts.modelOptions, ...opts.modelOptions } : this.opts.modelOptions;
    const sessionAffectingChange = opts.model !== void 0 || opts.voice !== void 0 || opts.language !== void 0 || opts.modelOptions !== void 0;
    this.opts = {
      ...this.opts,
      ...opts,
  ...
  stream(options) {
    const { connOptions = this.opts.connOptions ?? import_types.DEFAULT_API_CONNECT_OPTIONS } = options || {};
    ...
    const createRecvTask = async (signal) => {
      ...
      try {
        ...
        while (!this.closed && !signal.aborted) {
          ...
          const parsedServerEvent = import_api_protos.ttsKnownServerEventSchema.safeParse(result.value);
          if (!parsedServerEvent.success) {
            continue;
          }
          const serverEvent = parsedServerEvent.data;
          const sessionIdFromEvent = serverEvent.session_id;
      ...
      await this.tts.pool.withConnection(
        async (ws) => {
          try {
            ...
            try {
              await import_throws.ThrowsPromise.all(tasks.map((t) => t.result));
            } finally {
              inputSentEvent.set();
              await resourceCleanup();
