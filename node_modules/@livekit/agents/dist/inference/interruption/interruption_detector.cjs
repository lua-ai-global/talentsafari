...
class AdaptiveInterruptionDetector extends import_events.default {
  ...
  constructor(options = {}) {
    ...
    const lkBaseUrl = baseUrl ?? process.env.LIVEKIT_REMOTE_EOT_URL ?? (0, import_utils2.getDefaultInferenceUrl)();
    let lkApiKey = apiKey ?? "";
    let lkApiSecret = apiSecret ?? "";
    let useProxy;
    const isInferenceUrl = lkBaseUrl === import_utils2.DEFAULT_INFERENCE_URL || lkBaseUrl === import_utils2.STAGING_INFERENCE_URL;
    if (isInferenceUrl) {
  ...
  createStream() {
    try {
      ...
      const cause = (0, import_utils.asError)(e);
  ...
  async updateOptions(options) {
    ...
    await import_throws.ThrowsPromise.all(updatePromises);
  }
}
