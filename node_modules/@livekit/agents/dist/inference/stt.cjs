...
__export(stt_exports, {
  ...
  normalizeSTTFallback: () => normalizeSTTFallback,
  parseSTTModelString: () => parseSTTModelString
});
...
const DIARIZATION_EXTRA_KEYS = ["diarize", "speaker_labels", "diarization"];
function diarizationEnabled(extraKwargs) {
...
function parseSTTModelString(model) {
  ...
  if (idx !== -1) {
    return [model.slice(0, idx), (0, import_language.normalizeLanguage)(model.slice(idx + 1))];
  }
  return [model, void 0];
}
...
class STT extends import_stt.STT {
  ...
  constructor(opts) {
    const modelOptions = (opts == null ? void 0 : opts.modelOptions) ?? {};
    super({
      streaming: true,
      interimResults: true,
      alignedTranscript: "word",
      diarization: diarizationEnabled(modelOptions)
    });
    const {
      model,
      language,
    ...
    const lkBaseURL = baseURL || (0, import_utils2.getDefaultInferenceUrl)();
    ...
    if (typeof nextModel === "string") {
      ...
      if (parsedLanguage !== void 0) {
        if (nextLanguage && !(0, import_language.areLanguagesEquivalent)(nextLanguage, parsedLanguage)) {
          this.#logger.warn(
    ...
    this.opts = {
      model: nextModel,
      language: nextLanguage ? (0, import_language.normalizeLanguage)(nextLanguage) : void 0,
      encoding,
      sampleRate,
      baseURL: lkBaseURL,
      apiKey: lkApiKey,
      ...
      connOptions: connOptions ?? import_types.DEFAULT_API_CONNECT_OPTIONS
    };
  }
  get label() {
  ...
  updateOptions(opts) {
    const mergedModelOptions = opts.modelOptions ? { ...this.opts.modelOptions, ...opts.modelOptions } : this.opts.modelOptions;
    this.opts = {
      ...this.opts,
      ...opts,
    ...
    if (opts.modelOptions) {
      this.updateCapabilities({
        diarization: diarizationEnabled(this.opts.modelOptions)
      });
    }
  ...
  stream(options) {
    const { language, connOptions = this.opts.connOptions ?? import_types.DEFAULT_API_CONNECT_OPTIONS } = options || {};
    const streamOpts = {
      ...this.opts,
      language: language !== void 0 ? (0, import_language.normalizeLanguage)(language) : this.opts.language
    };
...
class SpeechStream extends import_stt.SpeechStream {
  ...
  async run() {
    while (true) {
      ...
      const send = async (socket, signal) => {
        ...
        const abortPromise = new import_throws.ThrowsPromise((_, reject) => {
          if (signal.aborted) {
        ...
        try {
          while (true) {
            const result = await import_throws.ThrowsPromise.race([iterator.next(), abortPromise]);
      ...
      const recv = async (signal) => {
        ...
        try {
          while (!this.closed && !signal.aborted) {
            ...
            const knownResult = import_api_protos.sttKnownServerEventSchema.safeParse(parseResult.data);
            ...
            switch (event.type) {
              ...
              case "interim_transcript":
                this.processTranscript(event, import_stt.SpeechEventType.INTERIM_TRANSCRIPT);
                break;
              case "final_transcript":
                this.processTranscript(event, import_stt.SpeechEventType.FINAL_TRANSCRIPT);
                break;
              case "preflight_transcript":
                this.processTranscript(event, import_stt.SpeechEventType.PREFLIGHT_TRANSCRIPT);
                break;
              case "error":
      ...
      try {
        ...
        const sendTask = import_utils.Task.from(({ signal }) => send(ws, signal), connController);
        const wsListenerTask = import_utils.Task.from(
          ({ signal }) => createWsListener(ws, signal),
          connController
        );
        const recvTask = import_utils.Task.from(({ signal }) => recv(signal), connController);
        const waitReconnectTask = import_utils.Task.from(
          ({ signal }) => import_throws.ThrowsPromise.race([this.reconnectEvent.wait(), (0, import_utils.waitForAbort)(signal)]),
          connController
        );
        try {
          await import_throws.ThrowsPromise.race([
            import_throws.ThrowsPromise.all([sendTask.result, wsListenerTask.result, recvTask.result]),
  ...
  processTranscript(data, eventType) {
    if (this.queue.closed) return;
    const requestId = data.session_id || this.requestId;
    const text = data.transcript;
    const language = (0, import_language.normalizeLanguage)(data.language || this.opts.language || "en");
    if (!text && eventType !== import_stt.SpeechEventType.FINAL_TRANSCRIPT) return;
    try {
      if (!this.speaking) {
        this.speaking = true;
      ...
      const metadata = extra && typeof extra === "object" && !Array.isArray(extra) && Object.keys(extra).length > 0 ? extra : void 0;
      const speechData = {
        language,
        ...
        words: data.words.map(
          (word) => (0, import_io.createTimedString)({
            text: word.word,
            startTime: word.start + this.startTimeOffset,
            endTime: word.end + this.startTimeOffset,
            startTimeOffset: this.startTimeOffset,
            confidence: word.confidence,
            speakerId: word.speaker_id ?? void 0
          })
        ),
        metadata
      };
      if (eventType === import_stt.SpeechEventType.FINAL_TRANSCRIPT) {
