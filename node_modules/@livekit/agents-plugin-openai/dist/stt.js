...
const unknownRealtimeTranscriptionServerEventSchema = z.object({
...
class SpeechStream extends stt.SpeechStream {
  ...
  async #forwardVadEvents(ws, vadStream) {
    for await (const event of vadStream) {
      ...
      } else if (event.type === VADEventType.END_OF_SPEECH && this.#options.turnDetection === null) {
        ws.send(JSON.stringify({ type: "input_audio_buffer.commit" }));
      }
    }
  }
