...
__export(realtime_model_exports, {
  ...
  livekitItemToOpenAIItem: () => livekitItemToOpenAIItem
});
  ...
  static withAzure({
    ...
    if (apiVersion) {
      (0, import_agents.log)().warn(
        "The `apiVersion` parameter for Azure OpenAI Realtime is deprecated and will be removed on April 30, 2026. Please use the newer Azure OpenAI Realtime API without specifying an API version."
      );
    }
    if (!baseURL) {
...
function normalizeAzureClientEvent(event) {
...
class RealtimeSession extends import_agents.llm.RealtimeSession {
  ...
  audioCapableItemIds = /* @__PURE__ */ new Set();
  ...
  hasInflightResponse() {
    return this.currentGeneration !== void 0 || Object.keys(this.responseCreatedFutures).length > 0;
  }
  sendEvent(command) {
  ...
  createSessionUpdateEvent() {
    ...
    const maxOutputTokens = opts.maxResponseOutputTokens === Infinity ? "inf" : opts.maxResponseOutputTokens;
    if (opts.isAzure && opts.apiVersion) {
      const modalities = opts.modalities.includes("audio") ? ["text", "audio"] : ["text"];
      return {
        type: "session.update",
        session: {
          model: opts.model,
    ...
    const modality = opts.modalities.includes("audio") ? "audio" : "text";
    return {
      type: "session.update",
      session: {
        type: "realtime",
  ...
  async updateChatCtx(_chatCtx) {
    ...
    try {
      const validation = import_agents.llm.validateChatContextStructure(_chatCtx);
      const blockingErrors = validation.issues.filter(
        (issue) => issue.severity === "error" && issue.code !== "timestamp_order"
      );
      const timestampOrderIssue = validation.issues.find(
        (issue) => issue.code === "timestamp_order"
      );
      ...
      const futures = [];
      const ownedCreateFutures = {};
      const ownedDeleteFutures = {};
      ...
      const timeoutPromise = (0, import_agents.delay)(5e3, { signal: timeoutController.signal }).then(() => {
  ...
  async #mainTask(signal) {
    ...
    while (!this.#closed && !signal.aborted) {
      ...
      try {
        ...
        const retryInterval = numRetries === 0 ? DEFAULT_FIRST_RETRY_INTERVAL_MS : this._options.connOptions.retryIntervalMs;
        this.#logger.warn(
          {
            attempt: numRetries,
            maxRetries,
  ...
  async runWs(wsConn) {
    const forwardEvents = async (signal) => {
      ...
      while (!this.#closed && wsConn.readyState === import_ws.WebSocket.OPEN && !signal.aborted) {
        try {
          ...
          if (this._options.isAzure && this._options.apiVersion) {
            normalizeAzureClientEvent(event);
          }
    ...
    const waitReconnectTask = import_agents.Task.from(async ({ signal }) => {
      await (0, import_agents.delay)(this._options.maxSessionDuration, { signal });
  ...
  handleConversationItemCreated(event) {
    ...
    const serverEventType = event.type;
  ...
  handleResponseAudioTranscriptDelta(event) {
    ...
    if (event.start_time !== void 0) {
      delta = (0, import_agents.createTimedString)({ text: event.delta, startTime: event.start_time });
    }
    const itemGeneration = this.currentGeneration.messages.get(itemId);
    if (!itemGeneration) {
      throw new Error("itemGeneration is not set");
