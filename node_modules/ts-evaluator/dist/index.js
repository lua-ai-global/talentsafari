  ...
  switch (operator) {
    ...
    case typescript.SyntaxKind.EqualsToken: {
      if (leftIdentifier != null) {
        ...
        const bestLexicalEnvironment = leftIdentifierValueDeclaration == null ? environment : findLexicalEnvironmentInSameContext(environment, leftIdentifierValueDeclaration, typescript) ?? environment;
...
var LAZY_CALL_FLAG = "___lazyCallFlag";
...
function stringifyLiteral(literal) {
  if (literal === void 0) return "undefined";
...
var NotCallableError = class extends EvaluationError {
  /**
...
function expressionContainsSuperKeyword(expression, typescript) {
  ...
  else if (typescript.isTypeAssertionExpression?.(expression) || !("isTypeAssertionExpression" in typescript) && typescript.isTypeAssertion(expression)) {
...
function evaluateCallExpression(options) {
  const { node, environment, evaluate: evaluate2, throwError, typescript, logger, getCurrentError } = options;
  const evaluatedArgs = [];
  for (let i = 0; i < node.arguments.length; i++) {
    const argument = node.arguments[i];
    if (argument == null) continue;
    evaluatedArgs[i] = evaluate2.expression(argument, options);
    if (getCurrentError() != null) {
      return;
    }
  }
  const expressionResult = evaluate2.expression(node.expression, options);
  if (getCurrentError() != null) {
    return;
  }
  if (isLazyCall(expressionResult)) {
    const currentThisBinding = expressionContainsSuperKeyword(node.expression, typescript) ? getFromLexicalEnvironment(node, environment, THIS_SYMBOL) : void 0;
    const value = expressionResult.invoke(currentThisBinding != null ? currentThisBinding.literal : void 0, ...evaluatedArgs);
    if (getCurrentError() != null) {
      return;
    }
    ...
    const value = typeof expressionResult !== "function" ? void 0 : maybeThrow(node, options, expressionResult(...evaluatedArgs));
    if (getCurrentError() != null) {
      return;
    }
    logger.logResult(value, "CallExpression");
...
function evaluateParenthesizedExpression({ node, evaluate: evaluate2, ...options }) {
  return evaluate2.expression(node.expression, options);
}
...
function hasModifier(node, modifier) {
  const modifiers = Array.isArray(node) ? node : "modifiers" in node && Array.isArray(node.modifiers) ? node.modifiers : void 0;
  return Boolean(modifiers?.some((m) => m.kind === modifier));
}
...
function evaluateParameterDeclarations(options, boundArguments, context) {
  const { node, evaluate: evaluate2, environment, typescript, getCurrentError } = options;
  ...
  for (let i = 0; i < parameters.length; i++) {
    ...
    if (parameter.dotDotDotToken != null) {
      evaluate2.nodeWithArgument(parameter, boundArguments.slice(i), options);
      if (getCurrentError() != null) {
        return;
      }
      break;
    } else {
      evaluate2.nodeWithArgument(parameter, boundArguments[i], options);
      if (getCurrentError() != null) {
        return;
      }
      if (context != null && typescript.isIdentifier(parameter.name) && (hasModifier(parameter, typescript.SyntaxKind.PublicKeyword) || hasModifier(parameter, typescript.SyntaxKind.ProtectedKeyword) || hasModifier(parameter, typescript.SyntaxKind.PrivateKeyword))) {
...
// src/interpreter/evaluator/evaluate-arrow-function-expression.ts
function evaluateArrowFunctionExpression(options) {
  const { node, environment, evaluate: evaluate2, stack, typescript, getCurrentError } = options;
  const arrowFunctionExpression = hasModifier(node, typescript.SyntaxKind.AsyncKeyword) ? async (...args) => {
    ...
    if (typescript.isBlock(node.body)) {
      evaluate2.statement(node.body, nextOptions);
      if (getCurrentError() != null) {
        return;
      }
      ...
      return evaluate2.expression(node.body, nextOptions);
    }
  } : (...args) => {
    const localLexicalEnvironment = cloneLexicalEnvironment(environment, node);
    const nextOptions = { ...options, environment: localLexicalEnvironment };
...
function evaluateObjectLiteralExpression(options) {
  const { node, evaluate: evaluate2, environment, getCurrentError } = options;
  ...
  for (const property of node.properties) {
    evaluate2.nodeWithArgument(property, value, options);
    if (getCurrentError() != null) {
      return;
    }
  }
...
function evaluateArrayLiteralExpression(options) {
  const { node, environment, evaluate: evaluate2, typescript, getCurrentError } = options;
  const arrayCtor = getFromLexicalEnvironment(node, environment, "Array").literal;
  const value = arrayCtor.of();
  for (const element of node.elements) {
    const nextValue = evaluate2.expression(element, options);
    if (getCurrentError() != null) {
      return;
    }
...
var UndefinedIdentifierError = class extends EvaluationError {
  constructor({ node, environment, message = `'${node.text}' is not defined'` }) {
    super({ message, environment, node });
  }
};

// src/interpreter/evaluator/evaluate-identifier.ts
function evaluateIdentifier(options) {
  const { node, environment, typeChecker, evaluate: evaluate2, stack, logger, reporting, typescript, throwError, getCurrentError } = options;
  ...
  let valueDeclaration = symbol == null ? void 0 : symbol.valueDeclaration;
  ...
  if (valueDeclaration != null) {
    if (valueDeclaration.getSourceFile().isDeclarationFile) {
      ...
      logger.logBinding(
        node.text,
        implementation,
        `Discovered declaration value${valueDeclaration.getSourceFile() === node.getSourceFile() ? "" : ` (imported into '${node.getSourceFile().fileName}' from '${valueDeclaration.getSourceFile().fileName}')`}`
      );
    ...
    evaluate2.declaration(valueDeclaration, options);
    if (getCurrentError() != null) {
      return;
    }
...
function evaluateBlock(options) {
  const { node, environment, typescript, evaluate: evaluate2, getCurrentError } = options;
  const localLexicalEnvironment = cloneLexicalEnvironment(environment, node);
  for (let i = 0; i < node.statements.length; i++) {
    const statement = node.statements[i];
    if (statement == null) continue;
    if (typescript.isConstructorDeclaration(node.parent) && i === 0 && typescript.isExpressionStatement(statement) && typescript.isCallExpression(statement.expression) && isSuperExpression(statement.expression.expression, typescript)) {
      continue;
    }
    evaluate2.statement(statement, { ...options, environment: localLexicalEnvironment });
...
function evaluateReturnStatement({ node, evaluate: evaluate2, stack, ...options }) {
  ...
  if (node.expression == null) {
    ...
    const result = evaluate2.expression(node.expression, options);
    if (getCurrentError() != null) {
      return;
    }
    stack.push(result);
...
function evaluateVariableDeclarationList({ node, evaluate: evaluate2, ...options }) {
  for (const declaration of node.declarations) {
    evaluate2.declaration(declaration, options);
...
// src/interpreter/evaluator/evaluate-prefix-unary-expression.ts
function evaluatePrefixUnaryExpression(options) {
  const { node, environment, evaluate: evaluate2, reporting, typescript, throwError, getCurrentError } = options;
  const operandValue = evaluate2.expression(node.operand, options);
  if (getCurrentError() != null) {
    return;
  }
...
function evaluatePropertyAccessExpression(options) {
  const { evaluate: evaluate2, node, statementTraversalStack, environment, typescript, getCurrentError } = options;
  const expressionResult = evaluate2.expression(node.expression, options);
  ...
  if (typeof match === "function" && statementTraversalStack.includes(typescript.SyntaxKind.CallExpression)) {
    return {
      ...
      invoke: (overriddenThis, ...args) => maybeThrow(
        node,
        options,
...
function evaluateElementAccessExpression(options) {
  const { node, environment, evaluate: evaluate2, statementTraversalStack, typescript, getCurrentError } = options;
  const expressionResult = evaluate2.expression(node.expression, options);
  if (getCurrentError() != null) {
    return;
  }
  const argumentExpressionResult = evaluate2.expression(node.argumentExpression, options);
  if (getCurrentError() != null) {
    return;
  }
  const match = node.questionDotToken != null && expressionResult == null ? (
...
function evaluateComputedPropertyName({ node, evaluate: evaluate2, ...options }) {
  return evaluate2.expression(node.expression, options);
}

// src/interpreter/evaluator/evaluate-function-declaration.ts
function evaluateFunctionDeclaration(options) {
  const { node, environment, evaluate: evaluate2, stack, typescript, getCurrentError } = options;
  const nameResult = node.name == null ? void 0 : node.name.text;
  const _functionDeclaration = hasModifier(node, typescript.SyntaxKind.AsyncKeyword) ? async function functionDeclaration(...args) {
    const localLexicalEnvironment = cloneLexicalEnvironment(environment, node);
    const nextOptions = { ...options, environment: localLexicalEnvironment };
    setInLexicalEnvironment({ ...nextOptions, path: RETURN_SYMBOL, value: false, newBinding: true });
    setInLexicalEnvironment({ ...nextOptions, path: "arguments", value: arguments, newBinding: true });
...
function evaluateIfStatement({ node, evaluate: evaluate2, ...options }) {
  const { getCurrentError } = options;
  const expressionValue = evaluate2.expression(node.expression, options);
  if (getCurrentError() != null) {
    return;
  }
  if (expressionValue) {
    evaluate2.statement(node.thenStatement, options);
    if (getCurrentError() != null) {
      return;
    }
  } else if (node.elseStatement != null) {
    return evaluate2.statement(node.elseStatement, options);
  }
  return;
}

// src/interpreter/evaluator/evaluate-expression-statement.ts
function evaluateExpressionStatement({ node, evaluate: evaluate2, stack, ...options }) {
  const result = evaluate2.expression(node.expression, options);
  if (options.getCurrentError() != null) {
    return;
  }
...
function evaluateTemplateExpression({ node, evaluate: evaluate2, ...options }) {
  let str = "";
  str += node.head.text;
  for (const span of node.templateSpans) {
    const expression = evaluate2.expression(span.expression, options);
    if (options.getCurrentError() != null) {
      return;
    }
    str += expression;
...
function evaluateTypeAssertion({ node, evaluate: evaluate2, ...options }) {
  return evaluate2.expression(node.expression, options);
}

// src/interpreter/evaluator/evaluate-postfix-unary-expression.ts
function evaluatePostfixUnaryExpression(options) {
  const { evaluate: evaluate2, node, environment, typescript, throwError, reporting } = options;
  evaluate2.expression(node.operand, options);
  switch (node.operator) {
    case typescript.SyntaxKind.PlusPlusToken: {
      if (!typescript.isIdentifier(node.operand) && !typescript.isPrivateIdentifier?.(node.operand)) {
        return throwError(new UnexpectedNodeError({ node: node.operand, environment, typescript }));
...
function evaluateNewExpression({ node, evaluate: evaluate2, ...options }) {
  const { getCurrentError } = options;
  const evaluatedArgs = [];
  if (node.arguments != null) {
    for (let i = 0; i < node.arguments.length; i++) {
      evaluatedArgs[i] = evaluate2.expression(node.arguments[i], options);
      if (getCurrentError() != null) {
        return;
      }
    }
...
function evaluateNonNullExpression({ node, evaluate: evaluate2, ...options }) {
  return evaluate2.expression(node.expression, options);
}

// src/interpreter/evaluator/evaluate-as-expression.ts
function evaluateAsExpression({ node, evaluate: evaluate2, ...options }) {
  return evaluate2.expression(node.expression, options);
}

// src/interpreter/evaluator/evaluate-switch-statement.ts
function evaluateSwitchStatement({ node, evaluate: evaluate2, ...options }) {
  const expressionResult = evaluate2.expression(node.expression, options);
  if (options.getCurrentError() != null) {
    return;
  }
  evaluate2.nodeWithArgument(node.caseBlock, expressionResult, options);
}

// src/interpreter/error/async-iterator-not-supported-error/async-iterator-not-supported-error.ts
var AsyncIteratorNotSupportedError = class extends EvaluationError {
  constructor({ message = `It is not possible to evaluate an async iterator'`, typescript, environment }) {
    super({
      message,
      environment,
      node: typescript.factory?.createEmptyStatement() ?? typescript.createEmptyStatement()
    });
  }
};

// src/interpreter/evaluator/evaluate-for-of-statement.ts
function evaluateForOfStatement(options) {
  const { node, environment, evaluate: evaluate2, logger, typescript, throwError, getCurrentError } = options;
  const expressionResult = evaluate2.expression(node.expression, options);
  if (getCurrentError() != null) {
    return;
  }
  ...
  if (node.awaitModifier != null) {
    ...
    for (const literal of expressionResult) {
      ...
      evaluate2.nodeWithArgument(node.initializer.declarations[0], literal, nextOptions);
      if (getCurrentError() != null) {
        return;
      }
      evaluate2.statement(node.statement, nextOptions);
      if (getCurrentError() != null) {
        return;
      }
...
function evaluateThisExpression({ node, environment }) {
  ...
  return match == null ? void 0 : match.literal;
}
...
// src/interpreter/evaluator/evaluate-for-statement.ts
function evaluateForStatement({ environment, evaluate: evaluate2, typescript, ...options }) {
  ...
  if (node.initializer !== void 0) {
    if (typescript.isVariableDeclarationList(node.initializer)) {
      for (const declaration of node.initializer.declarations) {
        evaluate2.declaration(declaration, forOptions);
        if (getCurrentError() != null) {
          return;
        }
      }
    } else {
      evaluate2.expression(node.initializer, forOptions);
      if (getCurrentError() != null) {
        return;
      }
    }
  ...
  while (true) {
    ...
    const conditionResult = node.condition == null ? true : evaluate2.expression(node.condition, forOptions);
    if (!conditionResult || getCurrentError() != null) return;
    evaluate2.statement(node.statement, iterationOptions);
    if (getCurrentError() != null) {
      return;
    }
    ...
    if (node.incrementor != null) {
      evaluate2.expression(node.incrementor, forOptions);
      if (getCurrentError() != null) {
        return;
      }
...
function evaluateWhileStatement(options) {
  const { node, environment, evaluate: evaluate2, logger, typescript, getCurrentError } = options;
  let condition = evaluate2.expression(node.expression, options);
  if (getCurrentError() != null) {
    return;
  }
  while (condition) {
    ...
    condition = evaluate2.expression(node.expression, options);
    if (getCurrentError() != null) {
      return;
    }
...
// src/interpreter/evaluator/evaluate-for-in-statement.ts
...
// src/interpreter/evaluator/evaluate-function-expression.ts
function evaluateFunctionExpression(options) {
  const { node, environment, evaluate: evaluate2, stack, typescript, getCurrentError } = options;
  const nameResult = node.name == null ? void 0 : node.name.text;
  const _functionExpression = hasModifier(node, typescript.SyntaxKind.AsyncKeyword) ? async function functionExpression(...args) {
    const localLexicalEnvironment = cloneLexicalEnvironment(environment, node);
    const nextOptions = { ...options, environment: localLexicalEnvironment };
    setInLexicalEnvironment({ ...nextOptions, path: RETURN_SYMBOL, value: false, newBinding: true });
    setInLexicalEnvironment({ ...nextOptions, path: "arguments", value: arguments, newBinding: true });
...
var MissingCatchOrFinallyAfterTryError = class extends EvaluationError {
...
function evaluateTryStatement(options) {
  const { node, evaluate: evaluate2, environment, throwError } = options;
  let error;
  const executeTry = () => {
    try {
      return evaluate2.statement(node.tryBlock, {
        ...options,
        throwError: (ex) => {
          error = ex;
          return ex;
  ...
  const executeCatch = (ex) => {
    evaluate2.nodeWithArgument(node.catchClause, ex, options);
  };
  const executeFinally = () => {
    let finallyError;
    evaluate2.statement(node.finallyBlock, {
      ...options,
      throwError: (ex) => {
        finallyError = ex;
...
function generateClassDeclaration({
  ...
  if (extendedType == null) {
    ...
    return new Function(
      "extendedType",
      "ctor",
      `return class ${name ?? ""} extends extendedType {constructor () {super(...arguments); const ctorReturnValue = ctor.call(this, ...arguments); if (ctorReturnValue != null) return ctorReturnValue;}}`
    )(extendedType, ctor);
  }
}
...
function getDecorators(node, typescript) {
  if ("getDecorators" in typescript) {
    ...
    const legacyDecorators = "decorators" in node && Array.isArray(node.decorators) ? node.decorators : void 0;
...
// src/interpreter/evaluator/evaluate-class-declaration.ts
function evaluateClassDeclaration(options) {
  const { node, evaluate: evaluate2, stack, logger, typescript, getCurrentError } = options;
  let extendedType;
  ...
  let ctor;
  if (ctorMember != null) {
    evaluate2.declaration(ctorMember, options);
    if (getCurrentError() != null) {
      return;
    }
    ctor = stack.pop();
  ...
  if (node.heritageClauses != null) {
    ...
    if (extendsClause != null) {
      ...
      if (firstExtendedType != null) {
        extendedType = evaluate2.expression(firstExtendedType.expression, options);
        if (getCurrentError() != null) {
          return;
        }
      }
  ...
  const name = node.name == null ? void 0 : node.name.text;
  ...
  if (canHaveDecorators(node, typescript)) {
    for (const decorator of getDecorators(node, typescript) ?? []) {
      evaluate2.nodeWithArgument(decorator, [classDeclaration], options);
      if (getCurrentError() != null) {
        return;
      }
  ...
  for (const member of otherMembers) {
    evaluate2.nodeWithArgument(member, hasModifier(member, typescript.SyntaxKind.StaticKeyword) ? classDeclaration : classDeclaration.prototype, options);
    if (getCurrentError() != null) {
      return;
    }
  }
...
// src/interpreter/evaluator/evaluate-constructor-declaration.ts
function evaluateConstructorDeclaration(options) {
  const { node, environment, evaluate: evaluate2, stack, getCurrentError } = options;
...
function evaluateSpreadElement({ node, evaluate: evaluate2, ...options }) {
  return evaluate2.expression(node.expression, options);
}

// src/interpreter/evaluator/evaluate-class-expression.ts
function evaluateClassExpression(options) {
  ...
  if (canHaveDecorators(node, typescript)) {
    for (const decorator of getDecorators(node, typescript) ?? []) {
      evaluate2.nodeWithArgument(decorator, [classExpression], options);
      if (getCurrentError() != null) {
        return;
      }
  ...
  for (const member of otherMembers) {
    evaluate2.nodeWithArgument(member, hasModifier(member, typescript.SyntaxKind.StaticKeyword) ? classExpression : classExpression.prototype, options);
    if (getCurrentError() != null) {
      return;
    }
  }
...
function evaluateVoidExpression({ node, evaluate: evaluate2, ...options }) {
  evaluate2.expression(node.expression, options);
  return void 0;
}

// src/interpreter/evaluator/evaluate-type-of-expression.ts
function evaluateTypeOfExpression({ evaluate: evaluate2, node, ...options }) {
  const result = evaluate2.expression(node.expression, options);
  if (options.getCurrentError() != null) {
    return;
  }
...
function evaluateEnumDeclaration(options) {
  ...
  for (const member of node.members) {
    evaluate2.nodeWithArgument(member, enumDeclaration, options);
    if (getCurrentError() != null) {
      return;
    }
  }
...
function evaluateSourceFileAsNamespaceObject(options) {
  const { node, evaluate: evaluate2, environment, typeChecker, stack, getCurrentError } = options;
  const objectCtor = getFromLexicalEnvironment(node, environment, "Object").literal;
...
function evaluateImportDeclaration({ node, evaluate: evaluate2, ...options }) {
  if (node.importClause == null) return;
  evaluate2.declaration(node.importClause, options);
}

// src/interpreter/evaluator/evaluate-throw-statement.ts
function evaluateThrowStatement({ node, evaluate: evaluate2, ...options }) {
...
var MaxOpDurationExceededError = class extends PolicyError {
  /**
...
async function evaluateAwaitExpression(options) {
  const { node, environment, evaluate: evaluate2, policy, throwError, getCurrentError } = options;
  const timeout = policy.maxOpDuration === Infinity ? void 0 : setTimeout(() => {
...
function evaluateConditionalExpression({ node, evaluate: evaluate2, ...options }) {
  const { getCurrentError } = options;
  const conditionValue = evaluate2.expression(node.condition, options);
  if (getCurrentError() != null) {
    return;
  }
  if (conditionValue) {
    return evaluate2.expression(node.whenTrue, options);
  }
  return evaluate2.expression(node.whenFalse, options);
}
...
function evaluateMethodDeclaration(options, parent) {
  const { node, environment, evaluate: evaluate2, stack, typescript, getCurrentError } = options;
  const nameResult = evaluate2.nodeWithValue(node.name, options);
  if (getCurrentError() != null) {
    return;
  }
  ...
  if (parent == null) {
    ...
    if (typescript.isClassLike(node.parent)) {
      evaluate2.declaration(node.parent, options);
      if (getCurrentError() != null) {
        return;
      }
      updatedParent = stack.pop();
    } else {
      updatedParent = evaluate2.expression(node.parent, options);
      if (getCurrentError() != null) {
        return;
      }
    }
  ...
  const _methodDeclaration = hasModifier(node, typescript.SyntaxKind.AsyncKeyword) ? async function methodDeclaration(...args) {
    const localLexicalEnvironment = cloneLexicalEnvironment(environment, node);
    const nextOptions = { ...options, environment: localLexicalEnvironment };
    setInLexicalEnvironment({ ...nextOptions, path: RETURN_SYMBOL, value: false, newBinding: true });
    setInLexicalEnvironment({ ...nextOptions, path: "arguments", value: arguments, newBinding: true });
  ...
  if (canHaveDecorators(node, typescript)) {
    for (const decorator of getDecorators(node, typescript) ?? []) {
      evaluate2.nodeWithArgument(decorator, [parent, nameResult], options);
      if (getCurrentError() != null) {
        return;
      }
      stack.pop();
    ...
    for (let i = 0; i < parameters.length; i++) {
      ...
      if (canHaveDecorators(parameter, typescript)) {
        for (const decorator of getDecorators(parameter, typescript) ?? []) {
          evaluate2.nodeWithArgument(decorator, [parent, nameResult, i], options);
          if (getCurrentError() != null) {
            return;
          }
          stack.pop();
...
function evaluatePropertyDeclaration({ node, evaluate: evaluate2, typescript, stack, ...options }, parent) {
  const { getCurrentError } = options;
  const propertyNameResult = evaluate2.nodeWithValue(node.name, options);
  if (getCurrentError() != null) {
    return;
  }
  if (parent == null) {
  ...
  parent[propertyNameResult] = node.initializer == null ? void 0 : evaluate2.expression(node.initializer, options);
  if (getCurrentError() != null) {
    return;
  }
  if (canHaveDecorators(node, typescript)) {
    for (const decorator of getDecorators(node, typescript) ?? []) {
      evaluate2.nodeWithArgument(decorator, [parent, propertyNameResult], options);
      if (getCurrentError() != null) {
        return;
      }
      stack.pop();
...
function evaluateImportClause({ node, evaluate: evaluate2, ...options }) {
  const { getCurrentError } = options;
  if (node.name != null) {
    evaluate2.declaration(node.name, options);
    if (getCurrentError() != null) {
      return;
    }
  }
  if (node.namedBindings != null) {
    if ("elements" in node.namedBindings) {
      for (const importSpecifier of node.namedBindings.elements) {
        evaluate2.declaration(importSpecifier, options);
        if (getCurrentError() != null) {
          return;
        }
      }
    } else {
      evaluate2.declaration(node.namedBindings.name, options);
    }
  }
}

// src/interpreter/evaluator/evaluate-import-specifier.ts
function evaluateImportSpecifier({ node, evaluate: evaluate2, ...options }) {
  evaluate2.declaration(node.propertyName ?? node.name, options);
}

// src/interpreter/evaluator/evaluate-namespace-import.ts
function evaluateNamespaceImport({ node, evaluate: evaluate2, ...options }) {
  evaluate2.declaration(node.name, options);
}

// src/interpreter/error/unexpected-syntax-error/unexpected-syntax-error.ts
var UnexpectedSyntaxError = class extends EvaluationError {
...
function evaluateNode({ node, ...rest }) {
  ...
  } else if (rest.typescript.isTypeAssertionExpression?.(node) || !("isTypeAssertionExpression" in rest.typescript) && rest.typescript.isTypeAssertion(node)) {
...
function evaluateBindingName({ node, evaluate: evaluate2, typescript, logger, ...options }, rightHandValue) {
  if (typescript.isIdentifier(node) || typescript.isPrivateIdentifier?.(node)) {
    ...
    evaluate2.nodeWithArgument(node, rightHandValue, options);
  }
}

// src/interpreter/evaluator/evaluate-set-accessor-declaration.ts
...
function evaluatePropertyAssignment({ node, evaluate: evaluate2, ...options }, parent) {
  const initializer = evaluate2.expression(node.initializer, options);
  if (options.getCurrentError() != null) {
    return;
  }
  const propertyNameResult = evaluate2.nodeWithValue(node.name, options);
...
function evaluateParameterDeclaration({ node, evaluate: evaluate2, logger, ...options }, boundArgument) {
  const boundValue = boundArgument != null || node.initializer === void 0 ? boundArgument : evaluate2.expression(node.initializer, options);
  if (options.getCurrentError() != null) {
    return;
  }
  logger.logBinding(node.name.getText(), boundValue, "evaluateParameterDeclaration");
  evaluate2.nodeWithArgument(node.name, boundValue, options);
}

// src/interpreter/evaluator/evaluate-shorthand-property-assignment.ts
function evaluateShorthandPropertyAssignment({ node, evaluate: evaluate2, ...options }, parent) {
  const { getCurrentError } = options;
  const identifier = node.name.text;
  const initializer = evaluate2.expression(node.name, options);
  if (getCurrentError() != null) {
    return;
  }
...
function evaluateSpreadAssignment({ node, evaluate: evaluate2, ...options }, parent) {
  const entries = evaluate2.expression(node.expression, options);
  if (options.getCurrentError() != null) {
    return;
  }
...
function evaluateArrayBindingPattern({ node, evaluate: evaluate2, ...options }, rightHandValue) {
  ...
  while (elementsCursor < node.elements.length) {
    ...
    evaluate2.nodeWithArgument(nextElement, value, options);
    if (options.getCurrentError() != null) {
      return;
    }
  }
...
// src/interpreter/evaluator/evaluate-binding-element.ts
function evaluateBindingElement(options, rightHandValue) {
  const { node, evaluate: evaluate2, logger, typescript, getCurrentError } = options;
  const bindingElementInitializer = node.initializer == null ? void 0 : evaluate2.expression(node.initializer, options);
  if (getCurrentError() != null) {
    return;
  }
  if ((typescript.isIdentifier(node.name) || typescript.isPrivateIdentifier?.(node.name)) && node.propertyName != null) {
    const aliasName = node.name.text;
    const propertyNameResult = evaluate2.nodeWithValue(node.propertyName, options);
    if (getCurrentError() != null) {
      return;
    }
    ...
    const propertyValue = typescript.isArrayBindingPattern(node.parent) ? rightHandValue : rightHandValue[propertyNameResult];
    const propertyValueWithInitializerFallback = propertyValue ?? bindingElementInitializer;
    evaluate2.nodeWithArgument(node.name, propertyValueWithInitializerFallback, options);
    if (getCurrentError() != null) {
      return;
    }
  } else if (!typescript.isIdentifier(node.name) && !typescript.isPrivateIdentifier?.(node.name) && node.propertyName == null) {
...
function evaluateObjectBindingPattern({ node, evaluate: evaluate2, ...options }, rightHandValue) {
  for (const element of node.elements) {
    evaluate2.nodeWithArgument(element, rightHandValue, options);
    if (options.getCurrentError() != null) {
      return;
    }
  }
...
function evaluateCaseBlock(options, switchExpression) {
  ...
  for (const clause of node.clauses) {
    evaluate2.nodeWithArgument(clause, switchExpression, nextOptions);
    if (getCurrentError() != null) {
      return;
    }
...
function evaluateCaseClause({ node, evaluate: evaluate2, ...options }, switchExpression) {
  const { getCurrentError } = options;
  const expressionResult = evaluate2.expression(node.expression, options);
  if (expressionResult !== switchExpression || getCurrentError() != null) return;
  for (const statement of node.statements) {
    evaluate2.statement(statement, options);
    if (getCurrentError() != null) {
      return;
    }
...
function evaluateCatchClause(options, ex) {
  ...
  if (node.variableDeclaration != null) {
    evaluate2.nodeWithArgument(node.variableDeclaration, ex, nextOptions);
    if (getCurrentError() != null) {
      return;
    }
  }
  evaluate2.statement(node.block, nextOptions);
}
...
function evaluateDecorator(options, [parent, propertyName, index]) {
  const { node, evaluate: evaluate2, environment, throwError, stack, getCurrentError } = options;
  const decoratorImplementation = evaluate2.expression(node.expression, options);
  if (getCurrentError() != null) {
    return;
  }
...
function evaluateEnumMember(options, parent) {
  const { node, typeChecker, evaluate: evaluate2, getCurrentError } = options;
  let constantValue = typeChecker?.getConstantValue(node);
  if (constantValue == null) {
    if (node.initializer != null) {
      constantValue = evaluate2.expression(node.initializer, options);
      if (getCurrentError() != null) {
        return;
      }
    } else {
      ...
      for (const sibling of [...beforeSiblings].reverse()) {
        ...
        if (sibling.initializer != null) {
          const siblingConstantValue = evaluate2.expression(sibling.initializer, options);
          if (getCurrentError() != null) {
            return;
          }
  ...
  const propertyName = evaluate2.nodeWithValue(node.name, options);
  if (getCurrentError() != null) {
    return;
  }
...
// src/interpreter/evaluator/evaluate-node-with-argument.ts
...
function evaluatePropertyName({ node, evaluate: evaluate2, typescript, ...options }) {
  return typescript.isComputedPropertyName(node) ? evaluate2.expression(node.expression, options) : typescript.isIdentifier(node) || typescript.isPrivateIdentifier?.(node) ? node.text : evaluate2.expression(node, options);
}
...
// src/interpreter/evaluator/node-evaluator/create-node-evaluator.ts
...
var LogLevelKind = /* @__PURE__ */ ((LogLevelKind2) => {
  LogLevelKind2[LogLevelKind2["SILENT"] = 0] = "SILENT";
  LogLevelKind2[LogLevelKind2["INFO"] = 1] = "INFO";
  LogLevelKind2[LogLevelKind2["VERBOSE"] = 2] = "VERBOSE";
  LogLevelKind2[LogLevelKind2["DEBUG"] = 3] = "DEBUG";
  return LogLevelKind2;
})(LogLevelKind || {});
...
// src/interpreter/logger/logger.ts
import { inspect } from "util";
import color from "ansi-colors";
var Logger = class {
  options;
  constructor(optionsOrLogLevel = {}) {
    const { logLevel = 0 /* SILENT */, color: { info = "white", verbose = "yellow", debug = "magenta" } = {} } = typeof optionsOrLogLevel === "object" ? optionsOrLogLevel : { logLevel: optionsOrLogLevel };
    this.options = {
      logLevel,
      color: {
        info,
   ...
   */
  logInfo(message) {
    if (this.options.logLevel < 1 /* INFO */) return;
   ...
   */
  logVerbose(message) {
    if (this.options.logLevel < 2 /* VERBOSE */) return;
   ...
   */
  logDebug(message) {
    if (this.options.logLevel < 3 /* DEBUG */) return;
...
function isExpression(node, typescript) {
  return typescript.isExpressionNode(node) || typescript.isIdentifier(node) || typescript.isPrivateIdentifier?.(node);
}
...
function evaluate({
  ...
  const policy = {
    ...
    process: {
      exit: typeof process2 === "boolean" ? process2 : process2.exit,
      spawnChild: typeof process2 === "boolean" ? process2 : process2.spawnChild
    }
  };
  const reporting = {
    ...reportingInput,
...
export {
  ...
  IoError,
  LogLevelKind,
  MaxOpDurationExceededError,
  MaxOpsExceededError,
  MissingCatchOrFinallyAfterTryError,
  ModuleNotFoundError,
  NetworkError,
  ...
  UndefinedLeftValueError,
  UnexpectedNodeError,
  UnexpectedSyntaxError,
  evaluate,
  isEvaluationError
};
